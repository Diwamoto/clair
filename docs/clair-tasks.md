...
| `E11` | `active` | `D4` | `E05`, `U05` | **P1**。syntax highlight を実ファイルで色として出す(仕様 §5.11)。現状 `SyntaxParser`(E05)には `ClairEditorLanguage` の外に呼び出し元が無く、`ClairEditorView.highlights` は常に空。日常的に使う言語の tree-sitter grammar を vendor し(Swift、Go、TypeScript/JS、Python、JSON、Markdown、Rust、shell を最低線)、parse 結果の capture を `EditorTokenKind` へ対応づけ、`EditorBuffers` から view へ渡す。`INV-PERF-005`(cancellable background、入力を止めない、欠けても正しさは損なわれない)と `INV-REV-002`(属性変更は revision を進めない)を満たすこと。差分 parse で 10 MiB fixture の打鍵が回帰しないことを計測する。**dogfood review (2026-09-22)**: 実アプリで highlight がまったく色として出ないことを再確認。fixture だけでなく Project から開いた各対象言語の実ファイルで capture→token→描画を通す UI regression test と目視確認を完了条件にする。 **性能計測 (2026-09-22、[結果](benchmarks/results/2026-09-22-iwamotodaikinoMac-mini/README.md))**: `make perf-budget` が `syntax.reset`/`syntax.update` を「UI 側に呼び出し元が無い」として失敗させている。実測では 4 MiB JSON の初回 parse が 175.8 ms、差分 parse が 4.13 ms なので、cancellable background + 差分 parse なら `BUDGET-OP-100` を満たせる。配線後に当該 2 件が `background` 以上になることを完了条件へ追加する。 |
...
| `H11` | `blocked` | `D4` | `H05`, `U04` | **P2**。AI provider の残り使用量を status footer に戻す。旧 Clair にあった provider quota/remaining 表示を、screen scraping ではなく provider の公式 API/hook または根拠のある usage event から取得する。provider ごとの window、使用量/残量、reset 時刻、取得時刻を正規化し、利用不可・未対応・認証切れ・stale を明示する。取得は background で rate-limit/cancel 可能にし、Project/terminal 切替を止めない。OpenCode/Codex/Claude Code のうち取得可能な provider を実データで検証し、取得不能な provider は偽の数値を出さない。 **進捗(2026-09-23)**: Codex を公式 `codex app-server` の `account/rateLimits/read`(stdio JSON-RPC、login shell 経由、12 s timeout)で取得し、`showQuota` ON の間だけ main actor 外で 5 分毎に更新(OFF で `.task` が cancel)。status bar は mock `QuotaMeter` どおり最も逼迫した窓を表示(実データで `Codex 5時間 残り15%`、offscreen 描画で確認)、tooltip に全 provider の window/残量/reset/取得時刻と stale(10 分超)・取得不可(未導入/認証エラー文言)・未対応を列挙。`ProviderQuotaTests` 4 件(live は `CLAIR_LIVE_QUOTA=1` で実 Codex から 5h 85%/7d 76% 取得を確認)、統合後 `ClairCoreTests` 220 件通過。**残(owner 判断待ち)**: (1) Claude Code は `rate_limits` を statusLine command にしか渡さないため、`claude` 起動に `--settings` で statusLine hook を差し込む必要があり ADR-0002(agent TUI を無改変で起動)に触れる。(2) OpenCode は Go plan の HTTP usage endpoint にユーザーの API key を送る必要がある。それまで両者は「未対応」と明示し数値を出さない。画面ロック中のため実 `.app` 目視は未。 |
...
- 完了 53 / 全 75(2026-09-22 時点)
- 残り 22(active 6、queued 15、`N08` は blocked)
