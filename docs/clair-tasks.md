...
| `U09` | `active` | `D2` | `U04` | **P1**。ターミナル/エディタ pane の境界(分割線)をドラッグして pane の大きさを変えられるようにする(2026-09-23 owner 依頼)。現状の分割線は 1pt の SwiftUI `DragGesture` で、隣の libghostty / editor の NSView に hit-test を奪われて実質掴めず、resize cursor も出ない。さらに `setRatio(splitContaining:)` が「first 側に id を含む split すべて」を書き換えるため、入れ子分割では外側と内側の比が一緒に動く。 |
