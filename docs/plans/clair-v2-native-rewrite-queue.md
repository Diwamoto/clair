...
| `E02` | `active` | `D5` | `E01` | Swift の text storage、line index、stable line ID、UTF-8/UTF-16/grapheme coordinate、immutable revision を実装する。randomized edit と differential tests が通ること。 |
...
| `T03` | `blocked` | `D4` | `T01`, `T02`, `T08` | macOS Ghostty surface を native workspace に接続する。local shell、selection、copy/paste、scrollback、font/DPI、window resize が動くこと。**Blocked evidence (2026-09-16)**: `ClairV2MacApp` に実PTY駆動のlocal shell window を実装し、keyboard入力、clipboard paste、bounded journal 上の scrollback cursor、font/DPI連動のreal PTY resizeは動作・12/12 test pass で確認済み(commit `5765f0c`)。しかし本来の acceptance である「Ghostty surface の接続」そのものは未達: `T01` の `ClairV2GhosttyABI` は `ghostty_init`/`ghostty_info`/`ghostty_config_*` のみを pin しており `ghostty_surface_*` 系の embedding entry point が無く、かつこの環境では libghostty 自体が未 vendor (`scripts/v2-ghostty.sh status` は `absent`)。そのため実際の glyph rendering と grid-aware selection（copyで実際の選択テキストを取得すること）が実装できない。ANSI/VT を独自解釈するfallback rendererは書いていない(旧 libvterm 経路の再発になるため)。copy操作は選択テキストを推測せずfail-closed。この ABI/vendor gap を `T01` の拡張 or 新規task として解消しない限り、`T05`/`T06`/`T07` も同じ理由で真の rendering ができない。 |
...
| `T08` | `active` | `D5` | `T01` | **(2026-09-16 追加)** `T03` が発見した gap を解消する: `ClairV2GhosttyABI` に `ghostty_surface_*`（surface作成、PTYバイト投入、cell grid/カーソル読み出し等、実際の embedding に必要な最小集合)の C ABI を追加 pin し、libghostty を実際に vendor する（`scripts/v2-ghostty.sh vendor` を実行可能にし、`status` が `absent` ではなく実バイナリを指すこと）。macOS で既知の PTY 出力を実際に surface へ流し込み、rendered cell grid（またはそれに相当する検証可能な出力）を読み戻せる minimal smoke が通ること。T01 と同様 D5 の invariants/failure modes/test matrix を先に書くこと。ライセンス/vendor 手順は T01 の記録を踏襲する。 |
...
| `U02` | `active` | `D3` | `U01` | canvas tokens を Swift の color/type/spacing/radius/motion primitives と reusable native controls に写す。値と semantic name の照合 test を持つこと。 |
