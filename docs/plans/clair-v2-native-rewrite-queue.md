...
| `V03` | `queued` | `D5` | `V01`, `V02` | `clair mcp serve` の stdio MCP adapter。`aiAvailable` な command のみ discovery/execution へ公開し、write/destructive/external は Clair GUI 上の承認後のみ実行。AI の自己申告 risk を authorization に使わない。threat test（`aiAvailable: false` の呼び出し拒否、承認なし destructive の拒否、別 user IPC 拒否）を完了条件に含む。D5 独立レビュー必須。 |
...
| `V05` | `queued` | `D4` | `V01`, `E04`, `V04` | Quick Open、全文検索・置換 UI（E04 の core に接続）、file watcher、agent による disk 変更の live reload（未保存 buffer は破棄、principle 8）、file 単位 local history と復元。10,000 file 規模で応答性を計測。 |
| `V06` | `queued` | `D5` | `V01`, `V04`, `H07`, `E09` | Mac の Git/worktree workflow。stage/unstage/commit、branch 切替、managed worktree の作成（repository 外の管理領域）、branch 全体 review（commit 済みと未 commit/untracked を分離）、adoption 前の clean commit 要求、merge commit、native merge editor または agent への conflict 再依頼、worktree/branch 削除の個別確認。Git なし Project では機能を出さない。D5 独立レビュー必須。 |
| `V07` | `queued` | `D4` | `V01`, `V04`, `T03` | agent launch profile（Claude Code/Codex/OpenCode）を raw terminal として Project root または managed worktree で複数起動。事実 signal（bell、exit code、公式 hook）のみ利用し TUI scraping しない。semantic adapter が失敗・切断しても `raw_terminal` と PTY が継続すること（ADR-0002）。mobile からの registered profile launch は本 task で protocol まで、UI は後続。 |
| `V08` | `queued` | `D3` | `V04`, `V07` | Mac の Project badge、通知 history、macOS 通知、Project/terminal 単位の mute。通知内容は事実 signal のみ、secret/terminal bytes を含めない。 |
| `V09` | `queued` | `D4` | `H01`, `T02` | ADR-0008 の Stable/Dev identity（別 bundle ID・別 data 領域）、ADR-0009 の GitHub Release + Ed25519 署名 update（自動 check・click 適用・backup/rollback）、update restart 時の PTY reattach、window close 後の daemon 継続と明示終了、agent 実行中の電源接続時 idle sleep 抑止（battery 時は設定）。 |
