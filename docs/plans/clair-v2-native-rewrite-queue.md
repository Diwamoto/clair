...
| `T05` | `blocked` | `D5` | `T01`, `T04`, `N01`, `T08` | iOS/iPadOS Ghostty surface と remote session attach を実装する。touch scroll/selection、hardware keyboard、safe-area/rotation、background detach が動くこと。**Blocked evidence (2026-09-18)**: `T03`/`T08` が macOS で使っている `ghostty_app_*`/`ghostty_surface_*` embedder API (`GhosttyKit.xcframework`) は、現在 pin している upstream commit `d4c88d8069912b653d707191388ca98e24751f12`(2026-09-15)の時点で **iOS ターゲットを提供しない**。pin 済み Zig 0.16.0 で `zig build -Doptimize=ReleaseFast -Demit-xcframework=true -Dtarget=aarch64-ios` を実行すると upstream 自身のビルドシステムが `error: iOS is not a supported target for the full Ghostty build; only libghostty-vt supports iOS (-Demit-lib-vt)`(`src/build/Config.zig:140`)で拒否することを実機確認済み。upstream の commit archaeology(`4f39c506e`/`396166ecb`/`b112f3954`、2026-08-12、`git compare` で pin の祖先と確認済み)により、iOS app/embedder target は pin を切る前に upstream 側で意図的に削除されていたと判明。`origin/main`(2026-09-18時点)でも復活していない。upstream が iOS 向けに提供するのは `libghostty-vt`(VT parsing のみ、`ghostty_app_new`/`ghostty_surface_new` 相当の PTY 起動・描画 API を持たない別ライブラリ)のみで、これは `ClairV2GhosttySurfaceView` が依存する embedder ABI の代替にならない。`Config/ghostty-pin.json` の `vendored_slices_note` に詳細記録。これは `scripts/v2-ghostty.sh vendor` を拡張すれば解決する実装課題ではなく、次のいずれかを選ぶ product/architecture 判断: (1) iOS 削除前の upstream commit へ re-pin(数週間分 stale、T03/T08 が現行 pin で検証済みの macOS 挙動を再検証する必要あり)、(2) vendored Ghostty source を fork/patch して iOS build target を復元(新たな supply-chain/保守負債)、(3) `libghostty-vt` を採用し Clair 独自の iOS terminal renderer(CoreText/Metal 等)を新規実装(`T08` が `T03` から切り出されたのと同様、新規 task として切り出すべき規模)、(4) 現時点で Phase 4 の「iOS/iPadOS Ghostty surface」の scope を明示的に見直す。いずれもユーザー判断待ちのため `T05` は `blocked` のまま。 |
