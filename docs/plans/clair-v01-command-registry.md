...
実装: `packages/ClairCore/Sources/ClairWorkspace/WorkbenchCommands.swift`(UI 非依存)、
projection: `ClairAppKit/ClairAppShell.swift`(`ClairWorkbenchStore`、palette、`ClairCommandMenu`)。

## State owner

MVP の state owner は **GUI process**(`ClairWorkbenchStore.state`)。daemon は PTY/session/journal を所有し、
