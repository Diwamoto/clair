...
`ClairAgent` accepts bounded OpenCode stdout chunks and emits only the
provider-independent `EventEnvelope<ClairAgentEventPayload>` model. H04
...
1. Every normalized event carries the exact H04 `ResourceScope` derived from
   the `ClairAgentSessionIdentity`; provider-supplied project, worktree, and
