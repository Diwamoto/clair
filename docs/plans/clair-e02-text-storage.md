...
`ClairEditorCore` は UI / v1 runtime へ依存しない SwiftPM library target とした。
...
```sh
swift build --package-path packages/ClairCore --target ClairEditorCore
swift test --package-path packages/ClairCore \
...
swift test -c release --package-path packages/ClairCore --filter EditorTextPerformanceTests
swift format lint --strict --recursive packages/ClairCore/Sources/ClairEditorCore \
  packages/ClairCore/Tests/ClairCoreTests/EditorText*.swift
swift format lint --strict packages/ClairCore/Sources/ClairEditorFixtures/EditorInvariants.swift
git diff --check
