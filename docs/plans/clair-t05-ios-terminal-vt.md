...
- **No fallback terminal engine.** Absence of the vendored artifact
  ...
  build, and any iOS build before `scripts/ghostty.sh vendor-vt` has
  run) fails every `GhosttyVTTerminal`/`ClairMobileTerminalSession` call
...
- **Separate vendored artifact, separate gating.** `GhosttyVT.xcframework`
  ...
  vendored (`scripts/ghostty.sh vendor` vs `vendor-vt`,
...
- **Pinned ABI subset, checked at compile time.** `ClairGhosttyVTABI`
...
- **Desktop-owned PTY geometry is never implicitly changed by this
  ...
  (`ClairMobileTerminalAttachment.size`, synced in
  `ClairMobileTerminalSession.foreground`), never the phone's screen
  size. `ClairTerminalView`'s safe-area/rotation handling only changes
...
- **Background detach releases the daemon-side subscriber slot, not just
  local state.** `ClairMobileTerminalSession.background()` calls
  `transport.detach` (T04's `ClairTerminalBoundary.detach`), which
...
- **Foreground reattach resumes from the last acknowledged cursor, and
  ...
  the retained window. `ClairMobileTerminalSession.foreground` tries the
...
- **No new Mac-side cell-grid readback API.** Reconnect/resync stays
  ...
  (`ClairTerminalJournal`/`ClairTerminalBoundary`); this task does not
  ...
  needed).
- **The mobile app never links host-only `ClairDaemonKit` code.**
  `ClairMobileTerminalAttachment` is a deliberate mobile-local mirror of
  the daemon-only `ClairTerminalAttachment`, the same pattern
  `ClairMobilePushRegistrationSnapshot` already established for H09's
...
- **Transport-neutral, matching this codebase's actual mobile-network
  ...
  `.unavailable`). `ClairMobileTerminalTransport` follows the same
  shape: a protocol mirroring `ClairTerminalBoundary`'s five operations,
  a `ClairMobileUnavailableTerminalTransport` stub, and (test-only) an
  in-process adapter wrapping a real `ClairTerminalBoundary` — proving
...
- **Hardware keyboard input is a minimal, standard VT100/xterm encoding,
  not libghostty-vt's Kitty-keyboard-protocol encoder.** `ClairTerminalKeyEncoding`
...
- **Testability of the highest-risk logic does not depend on a vendored
  iOS build.** `ClairMobileTerminalSession`'s attach/pump/gap-resync/
  background-detach/keyboard-input state machine is injectable
  (`ClairTerminalEngine` protocol, `GhosttyVTTerminal` is the only
  production conformance) so it is covered by a real `swift test` run on
  any host, against a real `ClairTerminalBoundary`, not just against a
  fake.
...
- **Plain-text formatting instead of manual cell-grid decoding.**
  ...
  `ClairTerminalView`'s CoreText renderer wants — the same shape E06/E08's
...
| `scripts/ghostty.sh vendor-vt` | build succeeds but artifact/headers missing | checks `libghostty-vt.a` exists per target before packaging, and `GhosttyVT.xcframework/Info.plist` exists after `xcodebuild -create-xcframework` |
| `Package.swift` | `GhosttyVTKit` (iOS-only slices) linked into a macOS target | every dependency edge/define/linker setting reaching it is `.when(platforms: [.iOS])`-gated; verified by a clean `swift build --package-path packages/ClairCore` (macOS host) with the artifact vendored |
...
| Availability | artifact absent (default state) | `GhosttyVTTerminal.isVendored == false`; every method throws `.runtimeUnavailable` (`ClairGhosttyVTTests`) |
...
| Handle lifetime | `@MainActor` class `deinit` touching a non-`Sendable` C pointer | `isolated deinit` (same fix T03 already made for `ClairGhosttySurfaceView`); caught by the iOS Simulator cross-build, not by the macOS-only build (documented explicitly so a future non-iOS-gated change does not silently reintroduce it) |
...
| Hardware keyboard | plain text / named keys / arrows / navigation / control chords | exact byte-for-byte encoding (`ClairTerminalKeyEncodingTests`, 7 tests) |
...
| Mobile/daemon boundary | mobile code accidentally imports `ClairDaemonKit` | impossible by construction: `ClairMobileKit`/`ClairTerminalView` do not depend on `ClairDaemonKit` in `Package.swift`; only the test fixture (`@testable import ClairDaemonKit` in `ClairCoreTests`) does |

## Boundary shape

```text
Config/ghostty-pin.json            + libghostty_vt block (separate artifact/flags from `build`)
scripts/ghostty.sh               + vendor-vt (iOS device+simulator, xcodebuild -create-xcframework)
packages/ClairCore
  Sources/ClairGhosttyVTABI       clair_ghostty_vt_abi.{h,c} — pinned libghostty-vt C ABI subset
  Sources/ClairGhosttyVT          GhosttyVTTerminal (@MainActor), ClairTerminalEngine protocol,
                                     GhosttyVTError, GhosttyVTScreenSnapshot/Cursor/ViewportPoint
  Sources/ClairTerminal           + ClairTerminalKeyEncoding (platform-neutral VT100/xterm encoder)
  Sources/ClairMobileKit          + ClairMobileTerminalTransport (protocol + Unavailable stub),
                                     ClairMobileTerminalSession (attach/pump/detach/resync/keys)
  Sources/ClairTerminalView       ClairTerminalView: iOS UIView (#if os(iOS)) — CoreText render,
...
transport adapter for `ClairMobileTerminalTransport` (mirrors the
...
- No iOS Simulator or device app actually ran `ClairTerminalView`
