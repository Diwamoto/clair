...
```sh
make check   # package manifest / v1 依存境界検証
make build   # core + apps target ビルド
make test    # Core / Apps package tests
```
...
xcodebuild \
  ...
  -derivedDataPath .build/xcode/mobile-simulator \
...
```sh
scripts/ghostty.sh status    # 現在の vendor 状態
scripts/ghostty.sh verify    # pin manifest / license / ABI 検証
scripts/ghostty.sh vendor    # 初回 or pin 更新時の fetch/build
```
...
実機 build、署名、APNs、TestFlight については [`native-mobile.md`](native-mobile.md) を参照する。これらは Apple Developer account 等の外部 credential が必要なため、自動化された daily 確認からは外す。
