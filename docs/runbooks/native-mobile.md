...
この runbook は `ClairMobile` の SwiftUI composition root を、Simulator、登録済みの
...
成功条件は `ClairMobileTests` と `ClairMobileUITests` が build/test され、root view の
...
2. `generic/platform=iOS` 向けに Release archive を作る。
   ...
   xcodebuild \
     ...
     -archivePath .build/xcode/v2-mobile/ClairMobile.xcarchive \
...
3. archive の `Info.plist` と署名を確認する。
   ...
   /usr/libexec/PlistBuddy \
     ...
     '.build/xcode/v2-mobile/ClairMobile.xcarchive/Info.plist'
   codesign --verify --deep --strict \
     '.build/xcode/v2-mobile/ClairMobile.xcarchive/Products/Applications/Clair v2 Mobile.app'
   ```
