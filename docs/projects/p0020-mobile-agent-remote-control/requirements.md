...
- `G-01`: Clairが管理する任意のPTY sessionを、iPhone/iPadからraw terminalとして閲覧・操作できる。
...
- Android、App Store一般配布、relayのSRE・課金・abuse prevention。
...
- 初期版はprivate-network onlyとし、Cloudflare One Client / Tunnel private routeを配布向けの既定経路、Tailscale
  Serveを自所有環境・開発用の経路として同じmobile APIへ接続する。public listenerとClair-owned relayは持たない。
...
- 外部に公開するのはlocalhost上の`clair-mobile-host`一つだけとし、`clair-ptyhost`はsame-user Unix socket/local IPC
  に閉じる。Tailscale/Cloudflareのnetwork identityだけでmobile APIを認証しない。
- Macで明示的にOrca型QR/deep-link pairingを開始し、device key、表示名、scope、作成日時、最終利用日時、revoke
  stateを端末単位で管理する。
- Pairing linkはendpoint、`host_id`、server identity/TLS fingerprint、protocol version、短時間だけ有効なone-time
  bootstrap secretを含む。device private key、発行済みdevice token、terminal content、prompt、cwd、credentialは
  含めない。
- Mobileはprotected storageにdevice keyと発行済みdevice tokenを保存し、以後の接続ではdevice ID、token、challenge
  proofを使う。host identity/fingerprintが同一ならendpoint変更後も再pairingしない。
...
- APNs payloadは内容を含まず、mobileがprivate channelを再接続するopaque wake identifierだけを送る。
...
- `FR-02`: initializeでprotocol major/minor、client identity、frame limit、capabilityをnegotiationし、major不一致を拒否する。
...
- `FR-11`: APNs通知、ログ、diagnosticにはterminal bytes、prompt、cwd、command line、credentialを含めない。
...
- `FR-15`: `clair-mobile-host`はmobile APIの唯一のremote boundaryとなり、`clair-ptyhost`をraw TCP/HTTP/WebSocket
  endpointとして公開しない。Tailscale/Cloudflare/relay adapterはtyped control/data protocolの外側でbyte streamを
  運ぶだけとする。
- `FR-16`: hostは明示操作ごとにone-time・短時間expiryのpairing linkを生成し、QR/deep linkでendpoint、host identity
  fingerprint、protocol version、bootstrap secretを伝える。未使用linkは一度だけ消費できる。
...
- `QR-02 Privacy`: mobile、APNs、ログ、crash reportへraw terminal、prompt、cwd、secretを永続化しない。
...
- `QR-08 Credential isolation`: pairing secret、device token、device private key、host private key、terminal contentは
  QR、ログ、metrics、crash reportに出さず、host/mobileのprotected storageに限定する。
- `QR-09 Transport neutrality`: Tailscale Serve、Cloudflare private route、将来relayで同じMobileControl APIのgolden
  fixtureと認証状態機械を再利用できる。transport固有の認証情報をoperation payloadへ混ぜない。
...
- `AC-06`: Project/session選択、current screen/bounded scrollback、raw input、interrupt、registered agent launchがprivate network上で動作する。
- `AC-07`: APNs通知にsecret/contentがなく、foreground復帰後にprivate channelからattentionを取得できる。
- `AC-08`: private TestFlight buildをGitHub Actionsのmain/manual/30日scheduleで生成できる。
...
- `AC-14`: `clair-ptyhost`がnetwork listenerを持たず、mobileは`clair-mobile-host`の単一typed endpointからのみ同じ
  session brokerへ到達する。Tailscale ServeとCloudflare private routeで同一protocol fixtureが通る。
...
- mobile branch review、diff/source editing、team role、Android。
