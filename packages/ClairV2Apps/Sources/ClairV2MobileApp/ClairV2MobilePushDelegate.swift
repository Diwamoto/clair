...
import Foundation
  ...
  /// `ClairV2MobileReconnectController`. All product logic -- the typed
  ...
  /// mapping -- lives in `ClairV2MobileKit` and is covered by
  /// `ClairV2CoreTests`. This type only forwards raw platform callbacks into
  ...
  final class ClairV2MobilePushDelegate: NSObject, UIApplicationDelegate {
    /// Set by `ClairV2MobileApp` immediately after both this delegate and
    ...
    /// through `EnvironmentValues.clairV2MobileReconnect`.
    var reconnect: ClairV2MobileReconnectController?
    ...
    /// (`ClairV2MobileNotificationCategory`) so a delivered notification's
    ...
    /// `ClairV2MobileReconnectController`'s verify-before-trust path, not a
    /// notification action shortcut.
    private func registerNotificationCategories() {
      #if canImport(UserNotifications)
        let categories = Set(
          ClairV2MobileNotificationCategory.allCases.map { category in
