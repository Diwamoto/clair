...
/// dispatching a scoped H06 command. The host's own `ClairAgentCommandBoundary`
...
public enum ClairMobileConversationError: Error, Equatable, LocalizedError, Sendable {
  ...
  case promptTooLarge
  case staleApproval
  case transportRejected(ClairAgentCommandError)
...
public struct ClairMobileConversationMessage: Equatable, Identifiable, Sendable {
...
/// exactly the fields H06 needs to build an `ClairAgentApprovalReference`;
/// it never carries provider-raw text or credentials.
public struct ClairMobileAttentionItem: Equatable, Identifiable, Sendable {
  ...
  func makeReference() throws -> ClairAgentApprovalReference {
    try ClairAgentApprovalReference(requestID: requestID, eventID: eventID, revision: revision)
  }
}
...
public struct ClairMobileConversationState: Equatable, Sendable {
  ...
  public private(set) var messages: [ClairMobileConversationMessage] = []
  public private(set) var toolCalls: [ClairAgentToolCallEvent] = []
  public private(set) var pendingApprovals: [ClairMobileAttentionItem] = []
  public private(set) var latestAttention: ClairAgentAttentionEvent?
  public private(set) var usage: ClairAgentUsageEvent?
  public private(set) var completion: ClairAgentCompletionStatus?
  ...
  @discardableResult
  public mutating func apply(_ event: ClairAgentNormalizedEvent) -> Bool {
  ...
  private mutating func fold(
    ...
    case .conversation(let message):
      messages.append(
        ClairMobileConversationMessage(
          id: eventID,
          role: message.role,
          text: message.text,
  ...
  /// `ClairAgentCommandBoundary.ingest` enforces on the host, so the UI
  /// never offers an approval the host would already reject as stale.
  private mutating func applyAttention(
    _ attention: ClairAgentAttentionEvent,
    eventID: EventID,
    revision: Revision
  ) {
...
/// exactly like `ClairMobileTransport`; this protocol only depends on
/// `ClairAgent`/`ClairTransport` types so the mobile app never links
/// host-only daemon code.
public protocol ClairMobileAgentTransport: Sendable {
  func dispatch(
...
public struct ClairMobileUnavailableAgentTransport: ClairMobileAgentTransport {
  public init() {}
...
public actor ClairMobileConversationController {
  ...
  /// foreground.
  @discardableResult
  public func ingest(_ event: ClairAgentNormalizedEvent) -> Bool {
  ...
  @discardableResult
  public func submitPrompt(_ text: String) async throws -> ClairAgentCommandOutcome {
    guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
      throw ClairMobileConversationError.emptyPrompt
    }
    guard text.utf8.count <= ClairAgentCommandAction.maximumPromptBytes else {
      throw ClairMobileConversationError.promptTooLarge
    }
    return try await dispatch(.prompt(text), key: "prompt")
  }

  @discardableResult
  public func approve(requestID: String) async throws -> ClairAgentCommandOutcome {
    try await respond(requestID: requestID, approve: true)
  }

  @discardableResult
  public func deny(requestID: String) async throws -> ClairAgentCommandOutcome {
    try await respond(requestID: requestID, approve: false)
  }

  @discardableResult
  public func interrupt() async throws -> ClairAgentCommandOutcome {
    try await dispatch(.interrupt, key: "interrupt")
  }

  @discardableResult
  public func stop() async throws -> ClairAgentCommandOutcome {
  ...
  private func respond(requestID: String, approve: Bool) async throws -> ClairAgentCommandOutcome
  {
    ...
    do {
      ...
      let action: ClairAgentCommandAction = approve ? .approve(reference) : .deny(reference)
      let outcome = try await dispatch(action, key: key)
      if outcome != .rejected {
        stateValue.removePendingApproval(requestID: requestID)
      }
      return outcome
    } catch ClairMobileConversationError.transportRejected(.staleApproval) {
  ...
  private func dispatch(
    ...
    let task = Task { [transport] () async throws -> ClairAgentCommandOutcome in
      ...
      do {
        ...
        throw ClairMobileConversationError.transportRejected(error)
      } catch {
        throw ClairMobileConversationError.transportFailed
      }
    }
    inFlight[key] = task
