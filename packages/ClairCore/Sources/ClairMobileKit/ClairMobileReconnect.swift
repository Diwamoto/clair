...
public enum ClairMobileReconnectError: Error, Equatable, LocalizedError, Sendable {
  case notPaired
...
/// (registered in `apple/ClairMobileApp/Info.plist`). Parsing only ever
...
/// fact. Only `ClairMobileReconnectController.handle(.launched(deepLink:))`
/// independently verifying it against the host's authoritative state may
/// promote it to something the app navigates to.
public struct ClairMobileDeepLink: Equatable, Sendable {
  ...
  public init(host: ClairHostID? = nil, scope: ResourceScope) throws {
    guard scope.isSessionScope else {
      throw ClairMobileReconnectError.invalidDeepLink
    }
    self.host = host
    self.scope = scope
  }
  ...
  public init(url: URL) throws {
    ...
    } catch is ClairMobileReconnectError {
      throw ClairMobileReconnectError.invalidDeepLink
    }
  }
}
...
/// could inspect.
public enum ClairMobileSceneEvent: Equatable, Sendable {
  case launched(deepLink: ClairMobileDeepLink?)
  case foregrounded
  case backgrounded
...
/// and `.verified` is only ever reached through `ClairMobileSessionVerifying`
/// -- never adopted directly from a cache or an unverified push/deep-link
/// payload.
public enum ClairMobileReconnectState: Equatable, Sendable {
  case idle
  case verifying(ResourceScope)
  case verified(ReplayCursor)
  case mustReconnect(ResourceScope?, ClairMobileReconnectError)
}
...
/// `ClairMobileAgentTransport`/`ClairMobileWorkspaceReading`'s role for
/// N05/N06: the production Network.framework/TLS adapter backed by H08's
/// `ClairSessionJournal.subscribe`/`snapshot` is a later integration
/// boundary, and this protocol deliberately depends only on
/// `ClairShared`/`ClairTransport` types so the mobile app can never link
/// host-only `ClairDaemonKit` code.
public protocol ClairMobileSessionVerifying: Sendable {
  func verify(
    scope: ResourceScope,
...
/// `ClairMobileUnavailableAgentTransport`/`ClairMobileUnavailableWorkspaceReading`.
...
/// trusting the cache.
public struct ClairMobileUnavailableSessionVerifying: ClairMobileSessionVerifying {
  public init() {}
...
/// `ClairDaemonKit`: the mobile app must never link host-only daemon code.
public struct ClairMobilePushRegistrationSnapshot: Equatable, Sendable {
  public let scope: ResourceScope
...
/// `ClairMobileSessionVerifying`, the concrete Network.framework/TLS
/// adapter is a later integration boundary.
public protocol ClairMobilePushRegistering: Sendable {
  func register(
...
public struct ClairMobileUnavailablePushRegistering: ClairMobilePushRegistering {
  public init() {}
...
public enum ClairMobilePushRegistrationState: Equatable, Sendable {
  case notRegistered
  case registering
  case registered(ClairMobilePushRegistrationSnapshot)
  case failed(ClairMobileReconnectError)
}
...
public enum ClairMobileNotificationCategory: String, CaseIterable, Equatable, Sendable {
...
/// routed through `ClairMobileSessionVerifying`, so the result is always
...
public actor ClairMobileReconnectController {
  private let verifier: any ClairMobileSessionVerifying
  private let pushRegistry: any ClairMobilePushRegistering
  private var connection: ClairAuthenticatedConnection?
  private var cachedCursor: ReplayCursor?
  private var stateValue: ClairMobileReconnectState = .idle
  private var pushRegistrationStateValue: ClairMobilePushRegistrationState = .notRegistered
  ...
  public init(
    verifier: any ClairMobileSessionVerifying = ClairMobileUnavailableSessionVerifying(),
    pushRegistry: any ClairMobilePushRegistering = ClairMobileUnavailablePushRegistering()
  ) {
    self.verifier = verifier
    self.pushRegistry = pushRegistry
  }

  public var state: ClairMobileReconnectState { stateValue }
  public var pushRegistrationState: ClairMobilePushRegistrationState {
  ...
  private func revalidateCache() async -> ClairMobileReconnectState {
  ...
  /// can act on it.
  private func resolve(_ deepLink: ClairMobileDeepLink) async -> ClairMobileReconnectState {
    guard let connection else {
  ...
  private func handlePushWake(_ pushEvent: ClairPushEvent) async -> ClairMobileReconnectState {
    do {
  ...
  private func verify(
    scope: ResourceScope,
    cachedCursor: ReplayCursor?
  ) async -> ClairMobileReconnectState {
    guard let connection else {
