...
///
/// - `ClairAgentRuntime` (H04) has no cap at all on the number of
///   concurrently spawned provider processes: every launched session is a
///   real OS process, file descriptor set, and process group.
/// - H08's `ClairSessionJournalLimits.maximumSubscribersPerSession` bounds
...
public struct ClairDaemonAgentSessionCounts: Codable, Equatable, Sendable {
  ...
  init(tallying sessions: [ClairAgentSessionSnapshot]) {
    var tally: [ClairAgentLifecycleState: Int] = [:]
...
/// - H04's `ClairAgentRuntime` starts with no sessions, so a pre-restart
///   `SessionID` is reported as unknown/stale rather than presented as
///   still running.
/// - H06's `ClairAgentCommandBoundary` and H08's `ClairSessionJournal`
...
public actor ClairDaemonHost {
  public let instanceID = UUID()
  public let workspace: ClairWorkspaceRuntime
  public let authority: ClairPairingAuthority
  public let agentRuntime: ClairAgentRuntime
  ...
  private struct SubscriberKey: Hashable {
    ...
    let subscriberID: ClairJournalSubscriberID
  }

  public init(
    workspace: ClairWorkspaceRuntime,
    authority: ClairPairingAuthority,
    agentRuntime: ClairAgentRuntime,
    ...
    // of the host.
    self.commandBoundary = try ClairAgentCommandBoundary(
  ...
  public func catalog(
  ...
  ) async throws -> ClairWorkspaceCatalog {
    do {
    ...
    var visible: [ClairProjectCatalogEntry] = []
    for project in full.projects {
      ...
      do {
        ...
        // scope restriction (see `ClairTransport.authorizeRead`), not just
    ...
    return ClairWorkspaceCatalog(projects: visible, isTruncated: full.isTruncated)
  }
  ...
  public func startSession(
    ...
    endpoint: (any ClairAgentCommandEndpoint)? = nil,
    on connection: ClairAuthenticatedConnection
  ...
  public func resumeSession(
    ...
    let existing: ClairAgentSessionSnapshot
    do {
  ...
  private func resolveAgentSessionSlot(
    _ slot: UUID, for snapshot: ClairAgentSessionSnapshot
  ) {
  ...
  private func rollbackFailedAttachment(
    _ snapshot: ClairAgentSessionSnapshot
  ) async {
  ...
  private func attach(
    snapshot: ClairAgentSessionSnapshot, endpoint: (any ClairAgentCommandEndpoint)?
  ) async throws {
    ...
    let resolvedEndpoint: any ClairAgentCommandEndpoint
    if let endpoint {
      ...
      resolvedEndpoint = ClairRawAgentEndpoint(snapshot: snapshot, process: rawProcess)
    }
  ...
  @discardableResult
  public func reapExitedSessions() async -> [ClairAgentSessionSnapshot] {
    let sessions = await agentRuntime.allSessions()
    var transitioned: [ClairAgentSessionSnapshot] = []
  ...
  public func subscribeJournal(
    ...
    cursor: ReplayCursor,
    on connection: ClairAuthenticatedConnection
  ) async throws -> ClairSessionJournalCatchUp {
  ...
  private func holdsAgentSessionSlot(_ lifecycle: ClairAgentLifecycleState) -> Bool {
    switch lifecycle {
  ...
  private func isTerminal(_ lifecycle: ClairAgentLifecycleState) -> Bool {
    switch lifecycle {
    case .stopped, .exited, .failed:
      true
    case .starting, .running, .stopping, .upgrading, .cleanupPending:
