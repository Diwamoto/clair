...
public enum ClairAgentNormalizedEventKind: String, Codable, CaseIterable, Sendable {
  case conversation
...
public enum ClairAgentConversationRole: String, Codable, Equatable, Sendable {
  case user
  case assistant
  case system
  case tool
...
public struct ClairAgentConversationEvent: Codable, Equatable, Sendable {
  public let role: ClairAgentConversationRole
  public let text: String
  public let isDelta: Bool

  public init(
    role: ClairAgentConversationRole,
    text: String,
    isDelta: Bool = true
  ) {
    self.role = role
...
public enum ClairAgentToolCallStatus: String, Codable, Equatable, Sendable {
  case started
  case running
  case completed
  case failed
...
public struct ClairAgentToolCallEvent: Codable, Equatable, Sendable {
  /// A stable digest of the provider call identity. Provider IDs are never
  /// copied into the normalized event model.
  public let toolID: String
  public let name: String
  public let status: ClairAgentToolCallStatus

  public init(
    toolID: String,
    name: String,
    status: ClairAgentToolCallStatus
  ) {
    self.toolID = toolID
    self.name = name
    self.status = status
...
public enum ClairAgentAttentionKind: String, Codable, Equatable, Sendable {
  case approval
  case question
  case authentication
  case input
...
public enum ClairAgentAttentionStatus: String, Codable, Equatable, Sendable {
  case pending
  case resolved
  case unknown
}

public struct ClairAgentAttentionEvent: Codable, Equatable, Sendable {
  ...
  public let kind: ClairAgentAttentionKind
  public let requestID: String?
  public let status: ClairAgentAttentionStatus

  public init(
    kind: ClairAgentAttentionKind,
    requestID: String?,
    status: ClairAgentAttentionStatus = .unknown
  ) {
    self.kind = kind
  ...
  public init(from decoder: Decoder) throws {
    ...
    self.init(
      kind: try container.decode(ClairAgentAttentionKind.self, forKey: .kind),
      requestID: try container.decodeIfPresent(String.self, forKey: .requestID),
      status: try container.decodeIfPresent(ClairAgentAttentionStatus.self, forKey: .status)
        ?? .unknown
    )
  }
...
public enum ClairAgentCompletionStatus: String, Codable, Equatable, Sendable {
  case succeeded
  case failed
  case cancelled
  case stopped
...
public struct ClairAgentCompletionEvent: Codable, Equatable, Sendable {
  public let status: ClairAgentCompletionStatus

  public init(status: ClairAgentCompletionStatus) {
    self.status = status
  }
}

public struct ClairAgentUsageEvent: Codable, Equatable, Sendable {
...
public enum ClairAgentEventPayload: Codable, Equatable, Sendable {
  case conversation(ClairAgentConversationEvent)
  case toolCall(ClairAgentToolCallEvent)
  case attention(ClairAgentAttentionEvent)
  case completion(ClairAgentCompletionEvent)
  case usage(ClairAgentUsageEvent)

  public var kind: ClairAgentNormalizedEventKind {
    switch self {
    case .conversation:
      .conversation
    case .toolCall:
  ...
  public init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: CodingKeys.self)
    let type = try container.decode(ClairAgentNormalizedEventKind.self, forKey: .type)
    switch type {
    case .conversation:
      self = .conversation(
        try container.decode(ClairAgentConversationEvent.self, forKey: .payload)
      )
    case .toolCall:
      self = .toolCall(
        try container.decode(ClairAgentToolCallEvent.self, forKey: .payload)
      )
    case .attention:
      self = .attention(
        try container.decode(ClairAgentAttentionEvent.self, forKey: .payload)
      )
    case .completion:
      self = .completion(
        try container.decode(ClairAgentCompletionEvent.self, forKey: .payload)
      )
    case .usage:
      self = .usage(
        try container.decode(ClairAgentUsageEvent.self, forKey: .payload)
      )
    }
  }
...
public typealias ClairAgentNormalizedEvent = EventEnvelope<ClairAgentEventPayload>

public enum ClairAgentStreamError: Error, Equatable, LocalizedError, Sendable {
  case invalidLimits
  case inputTooLarge
...
public struct ClairAgentEventStreamLimits: Codable, Equatable, Sendable {
  public static let hardMaximumInputBytes = ClairAgentLaunchLimits.hardMaximumOutputBytes
  ...
  public static let standard = try! Self(
    ...
    maximumInputBytes: ClairAgentLaunchLimits.standard.maximumOutputBytes,
    maximumEvents: 1_024,
  ...
  public init(
    ...
    guard maximumInputBytes > 0,
      ...
      throw ClairAgentStreamError.invalidLimits
    }
...
/// Incrementally decodes OpenCode's newline/SSE JSON stream into the event
...
public struct ClairOpenCodeStreamNormalizer: Sendable {
  public let identity: ClairAgentSessionIdentity
  public let epoch: SessionEpoch
  public let limits: ClairAgentEventStreamLimits
  ...
  private var pendingCompletion: (eventID: EventID, payload: ClairAgentCompletionEvent)?
  ...
  public init(
    ...
    limits: ClairAgentEventStreamLimits = .standard
  ) {
  ...
  public mutating func append(_ data: Data) throws -> [ClairAgentNormalizedEvent] {
    try ensureOpen()
    do {
  ...
  public mutating func append(
    ...
    guard !rawOutput.isTruncated else {
      failClosed()
      throw ClairAgentStreamError.providerOutputTruncated
    }
  ...
  public mutating func finish() throws -> [ClairAgentNormalizedEvent] {
    try ensureOpen()
    do {
      guard buffer.isEmpty else {
        throw ProtocolError.truncatedFrame
      }
      var events: [ClairAgentNormalizedEvent] = []
  ...
  private mutating func ensureOpen() throws {
    if finished {
      throw ClairAgentStreamError.streamFinished
    }
    guard !failed else { throw ClairAgentStreamError.streamFailed }
  }

  private mutating func appendChunk(_ data: Data) throws -> [ClairAgentNormalizedEvent] {
    guard !data.isEmpty else { return [] }
    guard data.count <= limits.maximumInputBytes - inputBytes else {
      throw ClairAgentStreamError.inputTooLarge
    }
  ...
  private mutating func consume(line originalLine: Data) throws -> [ClairAgentNormalizedEvent] {
    ...
    if line.starts(with: Data("id:".utf8)) {
      ...
      guard !trimmed.isEmpty,
        ...
        throw ClairAgentStreamError.invalidEventField
      }
    ...
    if payload == Data("[DONE]".utf8) {
      ...
      return try accept(
        ...
        semantics: [.completion(ClairAgentCompletionEvent(status: .succeeded))],
        eventIDs: [eventID]
      )
    }
  ...
  private mutating func accept(
    ...
    if let existing = seenEvents[providerKey] {
      guard existing == fingerprint else {
        throw ClairAgentStreamError.conflictingDuplicate
      }
      return []
    }
    ...
    for (originalIndex, semantic) in orderedSemantics {
      ...
      case .usage(let payload):
        guard !completionWasEmitted else {
          throw ClairAgentStreamError.eventAfterCompletion
        }
  ...
  private mutating func emit(
    eventID: EventID,
    payload: ClairAgentEventPayload
  ) throws -> ClairAgentNormalizedEvent {
    guard emittedEventCount < limits.maximumEvents else {
      throw ClairAgentStreamError.eventLimitExceeded
    }
...
private enum H05Semantic: Sendable, Equatable {
  ...
  var kind: ClairAgentNormalizedEventKind {
    switch self {
    case .conversation:
      .conversation
    case .toolCall:
...
private struct H05RawEvent: Decodable, Sendable {
  ...
  func semantics(limits: ClairAgentEventStreamLimits) throws -> [H05Semantic] {
    let type = try string(
      paths: [
        ["type"],
        ["event"],
  ...
  private func conversationEvent(
    type: String,
    limits: ClairAgentEventStreamLimits
  ) throws -> ClairAgentConversationEvent {
    ...
    let role = ClairAgentConversationRole(rawValue: roleValue ?? "assistant") ?? .unknown
    return ClairAgentConversationEvent(
      role: role,
      text: text,
  ...
  private func toolCallEvent(
    limits: ClairAgentEventStreamLimits
  ) throws -> ClairAgentToolCallEvent {
    let name = try string(
      paths: [
        ["tool_name"],
        ["name"],
    ...
    return ClairAgentToolCallEvent(
  ...
  private func toolStatus(
    limits: ClairAgentEventStreamLimits
  ) throws -> ClairAgentToolCallStatus {
  ...
  private func attentionEvent(
    type: String,
    limits: ClairAgentEventStreamLimits
  ) throws -> ClairAgentAttentionEvent {
    let kind: ClairAgentAttentionKind
    if [
    ...
    return ClairAgentAttentionEvent(
      kind: kind,
  ...
  private func completionEvent(
    type: String,
    limits: ClairAgentEventStreamLimits
  ) throws -> ClairAgentCompletionEvent {
    ...
    if type == "error" || status == "error" || status == "failed" || status == "failure" {
      return ClairAgentCompletionEvent(status: .failed)
    }
    if status == "cancelled" || status == "canceled" || status == "interrupt" {
      return ClairAgentCompletionEvent(status: .cancelled)
    }
    if status == "stopped" || status == "stop" {
      return ClairAgentCompletionEvent(status: .stopped)
    }
    if status == "success" || status == "succeeded" || status == "completed"
      || status == "complete" || status == "idle" || status == "done"
      || type == "done" || type == "complete" || type == "session.idle"
    {
      return ClairAgentCompletionEvent(status: .succeeded)
    }
    return ClairAgentCompletionEvent(status: .unknown)
  }

  private func usageEvent(
    type: String,
    limits: ClairAgentEventStreamLimits
  ) throws -> ClairAgentUsageEvent? {
    let usagePaths = [
      ["usage"],
      ["tokens"],
      ["part", "tokens"],
  ...
  private func unsignedInteger(
    ...
    switch value {
    case .integer(let value):
      guard value >= 0 else { throw ClairAgentStreamError.invalidEventField }
      return UInt64(value)
    case .double(let value):
