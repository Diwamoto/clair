  ...
  /// over `ClairMobileTerminalSession`. Draws whatever
  ...
  /// `PlatformColor`/`PlatformFont` from `ClairEditorView`), but far
  ...
  /// scrollback included via `ClairMobileTerminalSession.scroll`), so
  ...
  /// is plain text -- see `ClairGhosttyVTABI`'s header comment for why),
  ...
  /// PTY's actual size, synced by `ClairMobileTerminalSession.foreground`
  ...
  public final class ClairTerminalView: UIView, UIKeyInput {
    private let session: ClairMobileTerminalSession
    ...
    public init(
      session: ClairMobileTerminalSession,
    ...
    public required init?(coder: NSCoder) {
      fatalError("ClairTerminalView does not support coder-based restoration")
    }
    ...
    public override func paste(_ sender: Any?) {
      ...
      guard ClairTerminalPaste.needsConfirmation(text, modes: session.modes) else {
    ...
    public override func draw(_ rect: CGRect) {
      ...
      for index in 0..<rowsToDraw {
        ...
        for character in line {
          let width = ClairTerminalCellWidth.of(character)
          defer { column += width }
    ...
    @objc private func handlePan(_ gesture: UIPanGestureRecognizer) {
      ...
      case .changed:
        ...
        if deltaRows != 0, session.modes.mouseReporting {
          ...
          let point = cell(at: gesture.location(in: self))
          let wheel: ClairTerminalMouseButton = deltaRows > 0 ? .wheelUp : .wheelDown
          Task { [session] in
    ...
    private func send(_ key: ClairTerminalKey) {
