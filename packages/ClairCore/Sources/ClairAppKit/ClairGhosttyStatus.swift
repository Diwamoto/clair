...
/// `ClairGhostty` types directly everywhere. This never substitutes a
...
public struct ClairGhosttyStatus: Equatable, Sendable {
  ...
  @MainActor
  public static func current() -> ClairGhosttyStatus {
    let isVendored = GhosttyRuntime.isVendored
    do {
      try GhosttyRuntime.shared.activate()
      return ClairGhosttyStatus(isVendored: isVendored, activationError: nil)
    } catch {
      return ClairGhosttyStatus(isVendored: isVendored, activationError: String(describing: error))
    }
  }

  public var summary: String {
    guard isVendored else {
      return "Ghostty runtime not vendored (run scripts/ghostty.sh vendor)"
    }
