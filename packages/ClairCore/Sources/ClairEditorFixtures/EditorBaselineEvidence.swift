...
/// A recorded measurement from a pre-rewrite editor implementation, kept as a regression floor.
///
...
public enum BaselineSurface: String, Sendable, Hashable, CaseIterable {
  ...
  /// CodeMirror 6 in WKWebView, the v1 shipping default, removed.
...
/// Baseline evidence for the Clair native editor.
///
...
/// one job: recording the failure modes the rewrite is not allowed to reproduce.
public enum EditorBaselineEvidence {
  ...
  public static let all: [EditorBaselineMeasurement] = [
    ...
    // MARK: CodeMirror in WKWebView - the v1 default that removes.
    ...
    EditorBaselineMeasurement(
      ...
      value: 0.88,
      isFailure: false,
      note: "Extension-host API boundary, not key-to-glyph. Included to show the order of magnitude the rewrite is aiming at.",
      source: pocMeasurementsPath
    ),
    EditorBaselineMeasurement(
      id: "BASE-VS-002",
  ...
  /// The failure value a implementation must stay strictly better than for a given metric.
  ///
  /// Returns the *best* (lowest) recorded failure for that fixture/metric pair, so a result that
