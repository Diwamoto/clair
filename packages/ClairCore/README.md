...
'Sources/ClairShared/ClairProtocol.swift':
...
'ClairCoreTests' contains deterministic golden coverage for:
...
but no 'ClairCore' target imports them.

## H02 read-only workspace runtime

`ClairWorkspace` provides `ClairWorkspaceRuntime` for the Mac daemon's
read-only workspace surface. A caller registers a typed `ProjectID` with an
absolute `ClairProjectRoot`; the runtime discovers the repository root and
...
`ClairWorkspacePath` accepts only bounded relative paths. Resolution checks
...
`ClairTransport` owns the transport-neutral native client and host security
