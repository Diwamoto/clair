...
@testable import ClairWorkspace
  ...
  struct ClairTerminalHostTests {
    @Test func t02AuthenticatedHostControlsARealPTYWithoutFixtureEndpoint() async throws {
      try await withStack { stack in
        ...
        #expect(ready.found)
        let payload = try ClairTerminalInputRequest(
        ...
        let owner = try host.terminal.claimDesktopResizeOwner(
          scope: scope, generation: started.processGeneration)
        try host.terminal.resize(ClairTerminalSize(rows: 40, columns: 120), owner: owner)
        ...
        await #expect(throws: ClairTerminalError.staleEpoch) {
        ...
        #expect(throws: ClairTerminalError.resizeDenied) {
          try host.terminal.resize(ClairTerminalSize(), owner: owner)
        }
      }
    }
    ...
    @Test func t02HostDuplicateLaunchAndShutdownKeepOneProcessOwner() async throws {
      try await withStack { stack in
        ...
        await #expect(throws: ClairAgentError.duplicateLaunch(id)) {
  ...
  private struct T02HostStack {
    ...
    let target: ClairAgentTarget
    init(executable: String, arguments: [String]) throws {
      root = URL(fileURLWithPath: "/private/tmp/clair-t02-\(UUID().uuidString)")
      try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
      let project = try ProjectID("t02-project")
      target = ClairAgentTarget(projectID: project)
      let workspace = try ClairWorkspaceRuntime(projects: [
        ClairProjectRoot(id: project, rootURL: root)
      ])
      ...
      let provider = try ClairOpenCodeProvider(
        ...
        processFactory: ClairPTYAgentProcessFactory())
      let runtime = try ClairAgentRuntime(
        workspace: workspace, provider: provider,
        limits: ClairAgentLaunchLimits(terminationGracePeriod: 0.05))
    ...
    func receive(
      ...
      until needle: String
    ) async throws -> (found: Bool, cursor: ClairTerminalCursor) {
      var bytes = Data()
    ...
    func command(
      _ snapshot: ClairAgentSessionSnapshot, id: String, action: ClairAgentCommandAction
    ) throws -> ClairAgentCommand {
      try ClairAgentCommand(
