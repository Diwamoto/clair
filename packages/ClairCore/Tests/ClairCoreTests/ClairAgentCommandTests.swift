...
private final class H06Endpoint: ClairAgentCommandEndpoint, @unchecked Sendable {
  private let lock = NSLock()
  ...
  init(
    outcome: ClairAgentCommandOutcome = .committed, barrier: (@Sendable () -> Void)? = nil
  ) {
...
private struct H06Fixture: Sendable {
  ...
  static func make(
    ...
    limits: ClairAgentCommandLimits = .standard,
    ...
    let identity = try ClairAgentSessionIdentity(
      provider: ClairProviderIdentity(
        providerID: .openCode, version: ClairProviderVersion("h06")),
    ...
    let boundary = try ClairAgentCommandBoundary(authority: authority, limits: limits)
  ...
  func command(
    _ id: String, action: ClairAgentCommandAction = .prompt("private prompt"),
  ...
  func attention(
    ...
    status: ClairAgentAttentionStatus = .pending,
    eventID: String? = nil, kind: ClairAgentAttentionKind = .approval
  ) throws -> ClairAgentNormalizedEvent {
    try event(
      .attention(
        ClairAgentAttentionEvent(kind: kind, requestID: h06Digest(request), status: status)
  ...
  func pending(_ event: ClairAgentNormalizedEvent? = nil) throws -> ClairAgentApprovalReference
  {
    ...
    return try ClairAgentApprovalReference(
...
private func h06Reject(
  _ expected: ClairAgentCommandError, _ body: () async throws -> Void
) async {
  do {
    try await body()
    Issue.record("Expected rejection: \(expected.rawValue)")
  } catch let error as ClairAgentCommandError {
...
struct H06CommandTests {
  @Test(arguments: ClairAgentCommandKind.allCases)
  func h06RoutesEachCommandWithExactlyOnceEffects(kind: ClairAgentCommandKind) async throws {
    for worktree in [nil, "worktree-h06"] {
      let f = try await H06Fixture.make(worktree: worktree)
      let reference = try f.pending()
      let action: ClairAgentCommandAction =
        switch kind {
  ...
  func h06ConcurrentDuplicateAndDuplicateAuthorizationDispatchOnce() async throws {
    ...
    let results = try await withThrowingTaskGroup(of: ClairAgentCommandResult.self) { group in
      for _ in 0..<20 {
        group.addTask { try await f.boundary.dispatch(duplicateTicket, on: f.connection) }
      }
      var results: [ClairAgentCommandResult] = []
  ...
  @Test(arguments: [ClairAgentCommandOutcome.rejected, .indeterminate])
  func h06FailedOrUncertainOutcomesAreRetained(outcome: ClairAgentCommandOutcome) async throws {
  ...
  @Test(arguments: ClairAgentCommandKind.allCases)
  func h06CapabilitiesAreAuthoritativeAndDefaultDeny(kind: ClairAgentCommandKind) async throws {
  ...
  func h06ExactScopeAndPayloadBindingRejectWithoutEffects() async throws {
    ...
    let errors: [ClairAgentCommandError] = [
      ...
      for command in commands {
        group.addTask {
          do {
            ...
            #expect(error as? ClairAgentCommandError == .staleApproval)
            return false
          }
        }
      }
  ...
  func h06H05NormalizerSeparatesRequestedAndResolvedAttention() async throws {
    let f = try await H06Fixture.make()
    let events = try ClairOpenCodeStreamNormalizer.normalize(
      stdout: Data(
    ...
    #expect(
      try JSONDecoder().decode(ClairAgentAttentionEvent.self, from: oldWire).status == .unknown)
    #expect(f.endpoint.captured.isEmpty)
  }
  ...
  func h06UncorrelatedResolutionInvalidatesPendingApprovalWindow() async throws {
    ...
    var normalizer = ClairOpenCodeStreamNormalizer(
      identity: f.identity,
      epoch: f.epoch,
  ...
  func h06LifecycleAndGenerationRacesFenceOldTickets() async throws {
    ...
    #expect(throws: ClairAgentCommandError.staleSession) {
      try f.boundary.install(
  ...
  func h06InvalidStreamFencesEffects(reason: String) async throws {
    let f = try await H06Fixture.make()
    let reference = try f.pending()
    let event: ClairAgentNormalizedEvent
    switch reason {
    ...
    default:
      event = try EventEnvelope(
        ...
        payload: ClairAgentEventPayload.completion(.init(status: .failed)))
    }
  ...
  func h06StaleAndWrongScopeEventsDoNotPoisonCurrentAttachment() async throws {
    ...
    for scope in [
      ...
      #expect(throws: ClairAgentCommandError.invalidEventStream) { try f.boundary.ingest(event) }
    }
    _ = try await f.boundary.execute(
  ...
  func h06BoundsKeepOperationsAndAuditFiniteWithoutReexecution() async throws {
    let f = try await H06Fixture.make(
      limits: ClairAgentCommandLimits(maximumOperations: 2, maximumAuditEntries: 3))
  ...
  func h06PendingAndSessionLimitsFailClosed() async throws {
    let f = try await H06Fixture.make(
      limits: ClairAgentCommandLimits(maximumSessions: 1, maximumPendingApprovals: 1))
    _ = try f.pending()
    #expect(throws: ClairAgentCommandError.approvalCapacity) {
      try f.boundary.ingest(f.attention(revision: 2, request: "another"))
    }
    let another = try ClairAgentSessionIdentity(
      provider: f.identity.provider, projectID: f.identity.projectID, sessionID: SessionID("second")
    )
    #expect(throws: ClairAgentCommandError.sessionCapacity) {
      try f.boundary.install(
    ...
    #expect(f.endpoint.captured.isEmpty)
    #expect(throws: ClairAgentCommandError.invalidLimits) {
      _ = try ClairAgentCommandLimits(maximumOperations: 0)
    }
    #expect(throws: ClairAgentCommandError.invalidLimits) {
      _ = try ClairAgentCommandLimits(maximumAuditEntries: 4097)
    }
  }
  ...
  func h06PromptAndWireBoundsRejectBeforeAnyEffect() async throws {
    let f = try await H06Fixture.make()
    let maximum = String(repeating: "é", count: ClairAgentCommandAction.maximumPromptBytes / 2)
    _ = try await f.boundary.execute(f.command("max", action: .prompt(maximum)), on: f.connection)
    for invalid in [maximum + "é", " \n", "nul\0byte"] {
      #expect(throws: ClairAgentCommandError.invalidCommand) {
      ...
      #expect(throws: ClairAgentCommandError.invalidCommand) {
        _ = try JSONDecoder().decode(ClairAgentCommandAction.self, from: wire)
      }
    }
    #expect(throws: ClairAgentCommandError.invalidCommand) {
      _ = try JSONDecoder().decode(
        ClairAgentCommandAction.self, from: Data(#"{"kind":"stop","prompt":"secret"}"#.utf8))
    }
    #expect(throws: ClairAgentCommandError.invalidCommand) {
      _ = try ClairAgentCommandPayload(epoch: f.epoch, processGeneration: 0, action: .stop)
    }
    #expect(throws: ClairAgentCommandError.invalidCommand) {
      _ = try ClairAgentApprovalReference(
    ...
    let decoded = try ClairNativeTransportCodec.decodeFrame(ClairAgentCommand.self, from: wire)
