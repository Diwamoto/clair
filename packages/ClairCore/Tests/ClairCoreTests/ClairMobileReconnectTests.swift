...
/// initializer (available via `@testable import ClairTransport`), mirroring
...
/// A minimal `ClairMobileSessionVerifying` double. It never touches real
...
private actor N07MockVerifier: ClairMobileSessionVerifying {
  ...
  func verify(
    ...
    guard let cursorToReturn else {
      throw ClairMobileReconnectError.unknownDestination(scope)
    }
...
/// A minimal `ClairMobilePushRegistering` double.
private actor N07MockPushRegistry: ClairMobilePushRegistering {
  private(set) var registerCallCount = 0
  private(set) var unregisterCallCount = 0
  var snapshotToReturn: ClairMobilePushRegistrationSnapshot?
  var errorToThrow: Error?

  func setSnapshot(_ snapshot: ClairMobilePushRegistrationSnapshot?) {
...
func deepLinkParsesAFullyQualifiedSessionURL() throws {
  ...
  let link = try ClairMobileDeepLink(url: url)
...
func deepLinkRejectsAWrongScheme() {
  ...
  #expect(throws: ClairMobileReconnectError.invalidDeepLink) {
    try ClairMobileDeepLink(url: url)
  }
}
...
func deepLinkRejectsAMalformedIdentifier() {
  ...
  #expect(throws: ClairMobileReconnectError.invalidDeepLink) {
    try ClairMobileDeepLink(url: components.url!)
  }
}
...
func foregroundBackgroundForegroundWithNoNewEventsRestoresExactlyTheSameState() async throws {
  ...
  let controller = ClairMobileReconnectController(verifier: verifier)
...
func relaunchFromTerminatedViaDeepLinkIndependentlyVerifiesBeforeActing() async throws {
  ...
  let deepLink = try ClairMobileDeepLink(host: try ClairHostID("n07-host"), scope: scope)
...
func deepLinkForAnUnknownOrRevokedSessionIsRejectedWithATypedError() async throws {
  ...
  let deepLink = try ClairMobileDeepLink(scope: scope)
  let result = await controller.handle(.launched(deepLink: deepLink))
...
func deepLinkForADifferentHostIsRejectedWithoutContactingTheVerifier() async throws {
  ...
  let deepLink = try ClairMobileDeepLink(host: try ClairHostID("some-other-host"), scope: scope)
  let result = await controller.handle(.launched(deepLink: deepLink))
...
func launchWithoutADeepLinkOrCacheFailsClosedAsNotPaired() async throws {
  let controller = ClairMobileReconnectController()
...
func pushRegistrationSucceedsThroughTheInjectedTransport() async throws {
  ...
  let snapshot = ClairMobilePushRegistrationSnapshot(
    scope: scope, environment: .sandbox, expiresAt: Date(timeIntervalSince1970: 10_000),
    requiresRegistration: false
  )
  await registry.setSnapshot(snapshot)
  let controller = ClairMobileReconnectController(pushRegistry: registry)
  await controller.attach(connection: try n07Connection())
...
func pushEnvironmentMatchesTheBuildDistribution() {
  #expect(ClairMobileEnvironment.development.pushEnvironment == .sandbox)
  #expect(ClairMobileEnvironment.testFlight.pushEnvironment == .production)
}
...
func notificationCategoryMirrorsEveryPushEventKind() {
  for kind in [ClairPushEventKind.attention, .completion] {
    let category = ClairMobileNotificationCategory(kind: kind)
    switch kind {
    case .attention:
