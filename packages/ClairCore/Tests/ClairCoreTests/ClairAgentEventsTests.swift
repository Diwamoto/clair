...
private func h05Identity(worktree: String? = "worktree-h05") throws -> ClairAgentSessionIdentity {
  try ClairAgentSessionIdentity(
    provider: ClairProviderIdentity(
      providerID: .openCode,
      version: try ClairProviderVersion("h05-fixture")
    ),
...
func h05AcceptsBatchedRecordsLargerThanThePerRecordFrameBound() throws {
  let identity = try h05Identity()
  let limits = try ClairAgentEventStreamLimits(
...
func h05DeduplicatesIdenticalEventsAndRejectsConflictingReuse() throws {
  ...
  #expect(throws: ClairAgentStreamError.conflictingDuplicate) {
...
func h05EmitsCompletionAtFinishWhenUsageIsAbsentAndRejectsPostCompletionEvents() throws {
  ...
  #expect(throws: ClairAgentStreamError.streamFinished) {
...
func h05RejectsARecognizedEventAfterPublishedCompletion() throws {
  ...
  #expect(throws: ClairAgentStreamError.eventAfterCompletion) {
    try normalizer.append(h05Data(#"{"type":"text","delta":"late"}"# + "\n"))
  }
}
...
func h05RejectsMalformedAndIncompleteInput() throws {
  let identity = try h05Identity()
  var malformed = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  ...
  #expect(throws: ClairAgentStreamError.streamFailed) {
    try malformed.append(h05Data(#"{"type":"text","delta":"after-failure"}"# + "\n"))
  }

  var scalar = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  #expect(throws: ProtocolError.malformedPayload) {
  ...
  var invalidUTF8 = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  #expect(throws: ProtocolError.malformedPayload) {
  ...
  var incomplete = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  ...
  var emptyUsage = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  #expect(throws: ClairAgentStreamError.invalidEventField) {
...
func h05DoesNotCreateApprovalRequestsWithoutCorrelationIDs() throws {
  let identity = try h05Identity()
  var asked = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  #expect(throws: ClairAgentStreamError.invalidEventField) {
  ...
  var replied = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  ...
  var nested = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
...
func h05EnforcesInputRecordTextEventAndProviderBounds() throws {
  let identity = try h05Identity()
  let smallLimits = try ClairAgentEventStreamLimits(
  ...
  var recordBound = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1),
    limits: smallLimits
  )
  ...
  var inputBound = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1),
    limits: try ClairAgentEventStreamLimits(
  ...
  #expect(throws: ClairAgentStreamError.inputTooLarge) {
    try inputBound.append(Data(repeating: 0x20, count: 11))
  }

  var fieldBound = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1),
    limits: try ClairAgentEventStreamLimits(
      frameLimits: .standard,
  ...
  var eventBound = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1),
    limits: try ClairAgentEventStreamLimits(
      frameLimits: .standard,
  ...
  #expect(throws: ClairAgentStreamError.eventLimitExceeded) {
    try eventBound.append(h05Data(#"{"type":"text","event_id":"two","delta":"two"}"# + "\n"))
  }

  var truncatedOutput = ClairOpenCodeStreamNormalizer(
    identity: identity,
    epoch: try SessionEpoch(1)
  )
  #expect(throws: ClairAgentStreamError.providerOutputTruncated) {
    try truncatedOutput.append(
      rawOutput: ClairAgentRawOutput(stdout: h05Data("{}\n"), isTruncated: true)
    )
  }
}
...
func h05ValidatesStreamLimitsWhenDecodedFromJSON() throws {
  let valid = try ClairAgentEventStreamLimits(
  ...
  #expect(try JSONDecoder().decode(ClairAgentEventStreamLimits.self, from: encoded) == valid)

  let oversizedEvents = Data(
    """
    {"frame_limits":{"maximum_payload_bytes":128},"maximum_input_bytes":256,"maximum_events":\(ClairAgentEventStreamLimits.hardMaximumEvents + 1),"maximum_text_bytes":32,"maximum_identifier_bytes":16}
    """.utf8
  )
  #expect(throws: ClairAgentStreamError.invalidLimits) {
    try JSONDecoder().decode(ClairAgentEventStreamLimits.self, from: oversizedEvents)
  }

  let oversizedInput = Data(
    """
    {"frame_limits":{"maximum_payload_bytes":128},"maximum_input_bytes":\(ClairAgentEventStreamLimits.hardMaximumInputBytes + 1),"maximum_events":4,"maximum_text_bytes":32,"maximum_identifier_bytes":16}
    """.utf8
  )
  #expect(throws: ClairAgentStreamError.invalidLimits) {
    try JSONDecoder().decode(ClairAgentEventStreamLimits.self, from: oversizedInput)
  }
}
...
func h05ProducesStableWireOutputForTheSameInput() throws {
  ...
  let first = try ClairOpenCodeStreamNormalizer.normalize(
    stdout: input,
    identity: identity,
    epoch: try SessionEpoch(3),
    startingRevision: Revision(9)
  )
  let second = try ClairOpenCodeStreamNormalizer.normalize(
    stdout: input,
    identity: identity,
    epoch: try SessionEpoch(3),
    startingRevision: Revision(9)
