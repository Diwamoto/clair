...
private struct H08Fixture {
  ...
  static func make(
    ...
    let identity = try ClairAgentSessionIdentity(
      provider: ClairProviderIdentity(
        providerID: .openCode, version: ClairProviderVersion("h08")),
  ...
  func event(
    ...
    let payload = ClairAgentEventPayload.conversation(
      ClairAgentConversationEvent(role: .assistant, text: text)
    )
...
private func h08Reject(
  _ expected: ClairSessionJournalError, _ body: () throws -> Void
) {
  do {
    try body()
    Issue.record("Expected rejection: \(expected)")
  } catch let error as ClairSessionJournalError {
...
struct H08SessionJournalTests {
  ...
  func h08OpenReturnsInitialSnapshotAtStartingRevision() throws {
    ...
    let journal = ClairSessionJournal()
  ...
  func h08GapAppendFaultsTheJournalInsteadOfSilentlySkipping() throws {
    ...
    h08Reject(.journalFaulted) {
      _ = try journal.subscribe(
        try ClairJournalSubscriberID("sub"), cursor: try f.cursor(revision: 0))
    }
  }
  ...
  func h08RetryOfOldEventWithinRetentionButBeyondReplayHistoryIsDuplicateNotFault() throws {
    ...
    // buffer (`ClairSessionJournalLimits.defaultEventRetention`, 512
    ...
    let outcome = try journal.subscribe(
      try ClairJournalSubscriberID("innocent-bystander"), cursor: try f.cursor(revision: 0))
  ...
  func h08SubscribeAtHeadReturnsUpToDate() throws {
    ...
    _ = try journal.append(try f.event(revision: 1))
    let outcome = try journal.subscribe(
      try ClairJournalSubscriberID("sub"), cursor: try f.cursor(revision: 1))
  ...
  func h08SubscribeBehindHeadReplaysExactlyMissingEventsOnNetworkSwitch() throws {
    ...
    let outcome = try journal.subscribe(
      try ClairJournalSubscriberID("mobile-1"), cursor: try f.cursor(revision: 2))
  ...
  func h08RepeatedSubscribeBeforeAcknowledgeIsIdempotent() throws {
    ...
    let subscriberID = try ClairJournalSubscriberID("mobile-retry")
  ...
  func h08SlowSubscriberBeyondRetentionWindowGetsExplicitResync() throws {
    let f = try H08Fixture.make()
    let limits = try ClairSessionJournalLimits(eventRetention: 4)
    let journal = ClairSessionJournal(limits: limits)
    _ = try journal.open(identity: f.identity, epoch: f.epoch, processGeneration: 1)
    let subscriberID = try ClairJournalSubscriberID("slow-client")
    _ = try journal.append(try f.event(revision: 1))
  ...
  func h08OutOfRangeCursorAheadOfHeadIsRejectedNotAcceptedAsCaughtUp() throws {
    ...
    h08Reject(.invalidCursor) {
      _ = try journal.subscribe(
        try ClairJournalSubscriberID("ahead"), cursor: try f.cursor(revision: 99))
    }
  }
  ...
  func h08DaemonRestartWithFreshJournalRejectsStaleCursorExplicitly() throws {
    let f = try H08Fixture.make()
    let beforeRestart = ClairSessionJournal()
    ...
    let afterRestart = ClairSessionJournal()
    h08Reject(.unknownSession) {
      _ = try afterRestart.subscribe(
        try ClairJournalSubscriberID("survivor"), cursor: staleCursor)
    }
  }
  ...
  func h08NewProcessGenerationFencesOldSubscriberIntoResync() throws {
    ...
    _ = try journal.open(identity: f.identity, epoch: f.epoch, processGeneration: 1)
    _ = try journal.append(try f.event(revision: 1))
    let subscriberID = try ClairJournalSubscriberID("mobile-1")
  ...
  func h08AcknowledgeIsIdempotentAndMonotonic() throws {
    ...
    let subscriberID = try ClairJournalSubscriberID("acker")
    _ = try journal.subscribe(subscriberID, cursor: try f.cursor(revision: 0))
  ...
  func h08AcknowledgeRejectsARevisionNeverDelivered() throws {
    ...
    _ = try journal.open(identity: f.identity, epoch: f.epoch, processGeneration: 1)
    _ = try journal.append(try f.event(revision: 1))
    let subscriberID = try ClairJournalSubscriberID("acker-2")
    _ = try journal.subscribe(subscriberID, cursor: try f.cursor(revision: 0))
  ...
  func h08AcknowledgeForUnknownSubscriberIsRejected() throws {
    ...
    h08Reject(.unknownSubscriber) {
      try journal.acknowledge(
        try ClairJournalSubscriberID("never-subscribed"), scope: f.identity.sessionScope,
        upTo: .zero)
    }
  }
  ...
  func h08SessionCapacityIsBounded() throws {
    let limits = try ClairSessionJournalLimits(maximumSessions: 1)
    let journal = ClairSessionJournal(limits: limits)
  ...
  func h08SubscriberCapacityIsBoundedPerSession() throws {
    let f = try H08Fixture.make()
    let limits = try ClairSessionJournalLimits(maximumSubscribersPerSession: 1)
    let journal = ClairSessionJournal(limits: limits)
    _ = try journal.open(identity: f.identity, epoch: f.epoch, processGeneration: 1)
    _ = try journal.subscribe(
      try ClairJournalSubscriberID("a"), cursor: try f.cursor(revision: 0))
    h08Reject(.subscriberCapacity) {
      _ = try journal.subscribe(
        try ClairJournalSubscriberID("b"), cursor: try f.cursor(revision: 0))
    }
  ...
  func h08CloseThenSubscribeIsUnknownSession() throws {
    ...
    h08Reject(.unknownSession) {
      _ = try journal.subscribe(
        try ClairJournalSubscriberID("late"), cursor: try f.cursor(revision: 0))
    }
  }
  ...
  func h08ConcurrentAppendAndSubscribeNeverCorruptTheJournal() async throws {
    ...
    try await withThrowingTaskGroup(of: Void.self) { group in
      ...
      for index in 0..<8 {
        group.addTask {
          let subscriberID = try ClairJournalSubscriberID("concurrent-\(index)")
          for _ in 0..<20 {
            _ = try? journal.subscribe(subscriberID, cursor: try f.cursor(revision: 0))
          }
        }
