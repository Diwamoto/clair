...
/// initializer (available via `@testable import ClairV2Transport`) so the
...
/// A minimal `ClairV2MobileWorkspaceReading` double that counts calls and can
...
private actor N06MockReader: ClairV2MobileWorkspaceReading {
  ...
  var changedFileSummaryToReturn: ClairV2ChangedFileSummary
  var diffsByPath: [String: ClairV2GitDiff] = [:]
  var errorToThrow: Error?

  init(
    changedFileSummary: ClairV2ChangedFileSummary,
  ...
  func setDiff(_ diff: ClairV2GitDiff, forPath path: String) {
...
private func n06TextDiff(
  ...
  ClairV2GitDiff(
    path: try ClairV2WorkspacePath(path),
    basis: .workingTree,
    kind: .text,
    text: text,
    hunks: hunks,
...
func diffReviewAttachmentRejectsSessionScope() throws {
  ...
  #expect(throws: ClairV2MobileDiffReviewError.invalidScope(scope)) {
    _ = try ClairV2MobileDiffReviewController.Attachment(
...
func diffReviewOperationsRequireAttachment() async throws {
  let controller = ClairV2MobileDiffReviewController()
  #expect(await controller.isAttached == false)

  await #expect(throws: ClairV2MobileDiffReviewError.notAttached) {
    _ = try await controller.refreshChangedFiles()
  }
  await #expect(throws: ClairV2MobileDiffReviewError.notAttached) {
    _ = try await controller.selectFile(try ClairV2WorkspacePath("tracked.txt"))
  }
}
...
func diffReviewSelectFileRejectsUnknownPath() async throws {
  ...
  let controller = ClairV2MobileDiffReviewController(reader: reader)
  let scope = try ResourceScope(projectID: projectID)
  try await controller.attach(.init(scope: scope, connection: n06Connection()))
  _ = try await controller.refreshChangedFiles()

  let unknownPath = try ClairV2WorkspacePath("unknown.txt")
  await #expect(throws: ClairV2MobileDiffReviewError.fileNotFound(unknownPath)) {
...
// MARK: - Real H07 workspace + H03 authorization integration
  ...
  private struct N06WorkspaceBridge: ClairV2MobileWorkspaceReading {
    let authority: ClairPairingAuthority
    let runtime: ClairV2WorkspaceRuntime
  ...
  private struct N06HostFixture {
    ...
    static func make(
      ...
      let project = try ClairV2ProjectRoot(id: projectID, rootURL: rootURL)
    ...
    func makeController() -> ClairV2MobileDiffReviewController {
      ClairV2MobileDiffReviewController(reader: reader)
    }

    func attachment() throws -> ClairV2MobileDiffReviewController.Attachment {
  ...
  private final class N06GitFixture {
    ...
    init() throws {
      rootURL = URL(fileURLWithPath: "/private/tmp")
        .appendingPathComponent("clair-v2-n06-fixture-\(UUID().uuidString)", isDirectory: true)
  ...
  func diffReviewSurfacesBinaryFileExplicitly() async throws {
    ...
    _ = try await controller.refreshChangedFiles()
    let diff = try await controller.selectFile(try ClairV2WorkspacePath("binary.dat"))
  ...
  func diffReviewSurfacesTruncatedLargeDiffExplicitly() async throws {
    ...
    _ = try await controller.refreshChangedFiles()
    let diff = try await controller.selectFile(try ClairV2WorkspacePath("large.txt"))
  ...
  /// `ClairV2WorkspaceRuntime` (not a mock) through H03's real
  ...
  func diffReviewFullSessionProducesZeroGitMutations() async throws {
    ...
    let binaryDiff = try await controller.selectFile(try ClairV2WorkspacePath("binary.dat"))
    #expect(binaryDiff.kind == .binary)
    #expect(await controller.state.isBinary)
    _ = await controller.nextHunk()
    _ = await controller.previousHunk()

    let largeDiff = try await controller.selectFile(try ClairV2WorkspacePath("large.txt"))
    #expect(largeDiff.isTruncated)

    _ = try await controller.selectFile(try ClairV2WorkspacePath("untracked.txt"))
    ...
    async let first = controller.selectFile(try ClairV2WorkspacePath("tracked.txt"))
    async let second = controller.selectFile(try ClairV2WorkspacePath("tracked.txt"))
    _ = try await (first, second)
