...
private struct H09Fixture {
  ...
  func normalizedEvent(
    ...
    payload: ClairV2AgentEventPayload = .attention(
      ClairV2AgentAttentionEvent(kind: .approval, requestID: "opaque-request")
    )
  ) throws -> ClairV2AgentNormalizedEvent {
    try ClairV2AgentNormalizedEvent(
...
func h09PushEventProjectionDropsConversationAndPreservesOnlyOpaqueReferences() async throws {
  ...
  let conversation = try fixture.normalizedEvent(
    payload: .conversation(ClairV2AgentConversationEvent(role: .assistant, text: "private text"))
  )
  #expect(
...
func h09RegistryForwardsOnlyAttentionAndCompletionAndRechecksScope() async throws {
  ...
  _ = try await registry.send(
    fixture.pushEvent(
      id: "completion-h09",
      payload: .completion(ClairV2AgentCompletionEvent(status: .succeeded))
    ),
    to: fixture.deviceID,
    environment: .sandbox
  )
