...
/// A plain synchronous, lock-guarded flag. `ClairV2MobileConversationController
...
/// `ClairV2MobileClientTests`) until `signal` is set, proving the duplicate
...
/// A minimal `ClairV2MobileAgentTransport` double that counts dispatch calls
...
/// exactly-once ledger.
private actor N05MockTransport: ClairV2MobileAgentTransport {
  private(set) var callCount = 0
  private(set) var capturedActions: [ClairV2AgentCommandAction] = []
  ...
  var outcome: ClairV2AgentCommandOutcome
  var errorToThrow: ClairV2AgentCommandError?

  init(
    releaseGate: N05Gate? = nil,
    startedGate: N05Gate? = nil,
    outcome: ClairV2AgentCommandOutcome = .committed,
    errorToThrow: ClairV2AgentCommandError? = nil
  ) {
...
private final class N05Endpoint: ClairV2AgentCommandEndpoint, @unchecked Sendable {
  private let lock = NSLock()
  private var effects: [ClairV2AgentCommandEffect] = []
  let outcome: ClairV2AgentCommandOutcome

  init(outcome: ClairV2AgentCommandOutcome = .committed) {
    self.outcome = outcome
  }
...
/// Bridges the mobile-facing `ClairV2MobileAgentTransport` seam directly to
/// the real H06 `ClairV2AgentCommandBoundary`, so N05's own tests can prove
...
private struct N05BoundaryTransport: ClairV2MobileAgentTransport {
...
private struct N05Fixture: Sendable {
  ...
  static func make(
    ...
    let identity = try ClairV2AgentSessionIdentity(
      provider: ClairV2ProviderIdentity(
        providerID: .openCode, version: ClairV2ProviderVersion("n05")),
    ...
    let boundary = try ClairV2AgentCommandBoundary(authority: authority)
  ...
  func attachment(processGeneration: UInt64 = 1) throws
    -> ClairV2MobileConversationController
    .Attachment
  {
    try .init(
  ...
  func attentionEvent(
    ...
    status: ClairV2AgentAttentionStatus = .pending, eventID: String? = nil
  ) throws -> ClairV2AgentNormalizedEvent {
    try event(
      .attention(
        ClairV2AgentAttentionEvent(kind: .approval, requestID: n05Digest(request), status: status)
  ...
  func conversationEvent(
    ...
    try event(
      .conversation(ClairV2AgentConversationEvent(role: .assistant, text: text, isDelta: isDelta)),
      revision: revision
    )
  }
}
...
struct N05DuplicateTapTests {
  ...
  func duplicatePromptTapsJoinTheSameInFlightCommandInsteadOfDoubleDispatching() async throws {
    ...
    let controller = ClairV2MobileConversationController(transport: transport)
  ...
  func duplicateTapThroughTheRealH06BoundaryStillCommitsExactlyOnce() async throws {
    let f = try await N05Fixture.make()
    let controller = ClairV2MobileConversationController(transport: f.boundaryTransport)
    await controller.attach(try f.attachment())
    let pending = try f.attentionEvent(revision: 1)
...
struct N05BackgroundDeliveryTests {
  ...
  func eventsFoldedWhileBackgroundedAreFullyVisibleOnForegroundResume() async throws {
    let f = try await N05Fixture.make()
    let controller = ClairV2MobileConversationController(transport: N05MockTransport())
    await controller.attach(try f.attachment())
  ...
  func aNewProcessGenerationObservedWhileBackgroundedResetsTheTranscriptCleanly() async throws {
    ...
    let restarted = try EventEnvelope(
      ...
      payload: ClairV2AgentEventPayload.conversation(
        ClairV2AgentConversationEvent(role: .assistant, text: "after restart")
      )
    )
...
struct N05StaleApprovalTests {
  ...
  func approvingARequestNoLongerPendingLocallyIsRejectedWithoutCallingTransport() async throws {
    ...
    await #expect(throws: ClairV2MobileConversationError.staleApproval) {
  ...
  func anUncorrelatedResolutionInvalidatesTheWholeLocalPendingWindow() async throws {
    ...
    let uncorrelated = try f.event(
      .attention(ClairV2AgentAttentionEvent(kind: .approval, requestID: nil, status: .resolved)),
      revision: 3
    )
  ...
  func aCompletionEventClearsAllPendingApprovals() async throws {
    ...
    let completion = try f.event(
      .completion(ClairV2AgentCompletionEvent(status: .succeeded)), revision: 2
    )
  ...
  func detachRejectsFurtherCommandsWithoutReachingTransport() async throws {
    ...
    await #expect(throws: ClairV2MobileConversationError.notAttached) {
...
struct N05ConversationStateTests {
  ...
  func toolCallEventsUpdateByToolIDInsteadOfAppending() {
    var state = ClairV2MobileConversationState()
    let scope = try! ResourceScope(
      projectID: ProjectID("p"), sessionID: SessionID("s")
    )
    let epoch = try! SessionEpoch(1)
    func toolEvent(revision: UInt64, status: ClairV2AgentToolCallStatus)
      -> ClairV2AgentNormalizedEvent
    {
      try! EventEnvelope(
        eventID: EventID("tool-\(revision)"), kind: EventKind("agent.tool_call"),
        scope: scope, epoch: epoch, revision: Revision(revision),
        payload: .toolCall(
          ClairV2AgentToolCallEvent(toolID: "tool-1", name: "bash", status: status))
      )
    }
  ...
  func eventsOutsideTheAttachedScopeAreIgnored() {
    ...
    let foreign = try! EventEnvelope(
      ...
      payload: ClairV2AgentEventPayload.conversation(
        ClairV2AgentConversationEvent(role: .assistant, text: "should not appear")
      )
    )
