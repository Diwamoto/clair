...
// than `ClairV2GhosttyABI`'s `ghostty.h`/`GhosttyKit.xcframework`: T05
...
// `ClairV2Terminal`/`ClairV2TerminalStream` (T04) already provides and
// render the result with its own iOS `UIView` (`ClairV2TerminalView`),
...
//
// Like `ClairV2GhosttyABI`: when `CLAIR_GHOSTTY_VT_VENDORED` is defined
...
// materialized into `packages/ClairV2Core/Vendor/ghostty/` by
// `scripts/v2-ghostty.sh vendor-vt`), this header additionally includes
...
// line strings" shape `ClairV2TerminalView`'s CoreText renderer wants
...
// argument in the macOS ABI (`ClairV2GhosttyABI`'s `clair_ghostty_text_s`
...
// Same reasoning as `ClairV2GhosttyABI`: assigning the real upstream symbol
...
// expression (`ClairV2GhosttyABI`'s header comment explains the same
...
// `clair_ghostty_abi_is_vendored` (`ClairV2GhosttyABI`) exactly.
