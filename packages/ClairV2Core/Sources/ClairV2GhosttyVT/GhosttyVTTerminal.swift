...
/// `ClairV2TerminalView`'s CoreText renderer draws one `CTLine` per
/// element of, mirroring how `EditorLineRenderer` (`ClairV2EditorView`)
...
/// `GhosttyRuntime`/`GhosttySurfaceHandle` (`ClairV2Ghostty`): libghostty's
...
/// `ClairV2TerminalView` (a `UIView`, itself main-thread-only). Unlike
...
/// non-iOS build, and any iOS build before `scripts/v2-ghostty.sh
...
public final class GhosttyVTTerminal {
  ...
  // pattern for `ClairV2GhosttySurfaceView`'s handle/poll-timer cleanup;
  ...
  /// Feeds raw PTY output bytes (T04's `ClairV2TerminalStream` frames)
  ...
  /// `ClairV2TerminalView`, same "poll once per frame/write batch" pattern
