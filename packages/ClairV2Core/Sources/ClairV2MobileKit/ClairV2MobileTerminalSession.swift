...
/// The observable state of one `ClairV2MobileTerminalSession`. Mirrors the
/// shape of `ClairMobileClientState`/`ClairV2MobileReconnectState`: exactly
/// one of these holds at a time, and `.attached` is only ever reached
/// through a host-confirmed `attach()`, never assumed from a cache.
public enum ClairV2MobileTerminalSessionState: Equatable, Sendable {
  case detached
  case attaching
  case attached
  case failed
...
/// raw byte stream (via `ClairV2MobileTerminalTransport`), plus the
...
/// feature -- `ClairV2TerminalView` (iOS `UIView`) drives it and renders
...
/// test` run (see `ClairV2MobileTerminalSessionTests`) without a simulator.
///
...
/// daemon (T04's `ClairV2TerminalBoundary.detach`) instead of holding a
...
@MainActor
public final class ClairV2MobileTerminalSession {
  private let transport: any ClairV2MobileTerminalTransport
  private let terminal: any ClairV2TerminalEngine
  private var attachment: ClairV2MobileTerminalAttachment?
  private var lastAcknowledgedCursor: ClairV2TerminalCursor?
  ...
  public init(transport: any ClairV2MobileTerminalTransport, columns: Int = 80, rows: Int = 24)
    throws
  {
  ...
  /// Test/preview seam: injects a `ClairV2TerminalEngine` directly instead
  ...
  /// above.
  public init(transport: any ClairV2MobileTerminalTransport, engine: any ClairV2TerminalEngine) {
    self.transport = transport
  ...
  public func sendKey(_ key: ClairV2TerminalKey) async {
