...
/// failure, mirroring `ClairV2MobileConversationError`'s shape for N05
/// command dispatch.
public enum ClairV2MobileDiffReviewError: Error, Equatable, LocalizedError, Sendable {
  case notAttached
  case invalidScope(ResourceScope)
  case fileNotFound(ClairV2WorkspacePath)
...
/// already-computed `ClairV2ChangedFileSummary`/`ClairV2GitDiff` values.
public struct ClairV2MobileDiffReviewState: Equatable, Sendable {
  public private(set) var scope: ResourceScope?
  public private(set) var changedFiles: ClairV2ChangedFileSummary?
  public private(set) var selectedPath: ClairV2WorkspacePath?
  public private(set) var diff: ClairV2GitDiff?
  public private(set) var hunkIndex: Int?

  public init() {}

  public var files: [ClairV2ChangedFile] { changedFiles?.files ?? [] }
  public var hunks: [ClairV2GitDiffHunk] { diff?.hunks ?? [] }

  public var currentHunk: ClairV2GitDiffHunk? {
  ...
  mutating func applyChangedFiles(_ summary: ClairV2ChangedFileSummary) {
  ...
  mutating func applyDiff(_ diff: ClairV2GitDiff, path: ClairV2WorkspacePath) {
    selectedPath = path
    self.diff = diff
  ...
  /// `ClairV2MobileConversationController.submitPrompt`, so this stays a
...
/// mirroring `ClairV2MobileAgentTransport`'s role for H06 commands. The
/// production Network.framework/TLS adapter is a later integration boundary;
/// this protocol only depends on `ClairV2Workspace`'s Codable result types.
...
/// computed by H07's read-only `ClairV2WorkspaceRuntime`.
public protocol ClairV2MobileWorkspaceReading: Sendable {
...
/// `ClairV2MobileUnavailableAgentTransport`. The native app can construct a
...
public struct ClairV2MobileUnavailableWorkspaceReading: ClairV2MobileWorkspaceReading {
  public init() {}
...
public actor ClairV2MobileDiffReviewController {
  ...
  public struct Attachment: Equatable, Sendable {
    ...
    public init(scope: ResourceScope, connection: ClairAuthenticatedConnection) throws {
      guard !scope.isSessionScope else {
        throw ClairV2MobileDiffReviewError.invalidScope(scope)
      }
      self.scope = scope
  ...
  private let reader: any ClairV2MobileWorkspaceReading
  private var attachment: Attachment?
  private var stateValue = ClairV2MobileDiffReviewState()
  private var inFlightChangedFiles: Task<ClairV2ChangedFileSummary, Error>?
  private var inFlightDiffs: [String: Task<ClairV2GitDiff, Error>] = [:]

  public init(
    reader: any ClairV2MobileWorkspaceReading = ClairV2MobileUnavailableWorkspaceReading()
  ) {
    self.reader = reader
  }

  public var state: ClairV2MobileDiffReviewState { stateValue }
  ...
  @discardableResult
  public func refreshChangedFiles() async throws -> ClairV2ChangedFileSummary {
    guard let attachment else { throw ClairV2MobileDiffReviewError.notAttached }
    if let existing = inFlightChangedFiles {
      return try await existing.value
    }
    let task = Task { [reader] () async throws -> ClairV2ChangedFileSummary in
      do {
        ...
        throw ClairV2MobileDiffReviewError.transportFailed
      }
    }
  ...
  public func selectFile(
    ...
    {
      throw ClairV2MobileDiffReviewError.fileNotFound(path)
    }
    let key = "\(basis.rawValue):\(path.rawValue)"
    if let existing = inFlightDiffs[key] {
      return try await existing.value
    }
    let task = Task { [reader] () async throws -> ClairV2GitDiff in
      do {
