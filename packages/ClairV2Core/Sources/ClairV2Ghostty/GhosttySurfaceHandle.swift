...
/// dead-key handling) can distinguish them, matching `ClairV2GhosttySurfaceView`'s
...
/// `ClairV2AppKit` never needs `ghostty_input_key_e`'s large enum mirrored
...
/// modeled — `ClairV2GhosttySurfaceView` only wires primary/secondary/middle
/// click today.
...
public final class GhosttyAppHandle {
  ...
  /// `ClairV2GhosttySurfaceView`) spans many run-loop turns — keystrokes,
...
public final class GhosttySurfaceHandle {
  ...
  /// in `ClairV2AppKit`, handed here) rather than a per-character
  ...
  /// corner (matching `ClairV2GhosttySurfaceView.isFlipped == true`, so
