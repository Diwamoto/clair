...
import Foundation
  ...
  public enum ClairV2LocalShellError: Error, Equatable, Sendable {
    case alreadyStarted
    case notRunning
  ...
  /// This is deliberately independent of `ClairV2Agent`'s launch-spec/session
  ...
  public struct ClairV2LocalShellSpec: Sendable {
  ...
  public typealias ClairV2LocalShellTerminationHandler = @Sendable (ClairV2LocalShellExit) -> Void

  public struct ClairV2LocalShellExit: Equatable, Sendable {
  ...
  /// path (T02's `ClairV2TerminalBoundary`/`ClairV2PTYAgentProcess` own that,
  /// for T04/T06 remote surfaces); it reuses the same bounded
  /// `ClairV2TerminalJournal` and the same `ClairV2PTY` C spawn/resize
  /// primitives so a real PTY, not an emulated one, backs the surface.
  public final class ClairV2LocalShellSession: ClairV2TerminalProcess, @unchecked Sendable {
    public let terminalJournal: ClairV2TerminalJournal
    private let lock = NSLock()
    private let spec: ClairV2LocalShellSpec
    private let cwd: Int32
    private let limits: ClairV2TerminalLimits
    private let onTermination: ClairV2LocalShellTerminationHandler
    ...
    public init(
      spec: ClairV2LocalShellSpec = .loginShell(),
      limits: ClairV2TerminalLimits = .standard,
      size: ClairV2TerminalSize = try! ClairV2TerminalSize(),
      onTermination: @escaping ClairV2LocalShellTerminationHandler = { _ in }
    ) throws {
      self.spec = spec
      self.limits = limits
      self.size = size
      ...
      guard descriptor >= 0 else {
        throw ClairV2LocalShellError.workingDirectoryUnavailable(spec.workingDirectoryURL)
      }
      self.cwd = descriptor
    }
    ...
    public var terminalSize: ClairV2TerminalSize { lock.withLock { size } }

    public func start() throws {
      try lock.withLock {
        guard !started else { throw ClairV2LocalShellError.alreadyStarted }
        started = true
        ...
        guard result == 0 else {
          ...
          throw ClairV2LocalShellError.launchFailed
        }
        handle = spawned
        running = true
        let owned = spawned
    ...
    /// thrown error instead of inspecting `ClairV2TerminalCommitOutcome`.
    public func write(_ bytes: Data) throws {
      guard enqueueTerminalInput(bytes) == .queued else {
        throw ClairV2LocalShellError.inputRejected
      }
    }

    public func enqueueTerminalInput(_ bytes: Data) -> ClairV2TerminalCommitOutcome {
      lock.withLock {
    ...
    public func commitTerminalSignal(_ signal: Int32) -> ClairV2TerminalCommitOutcome {
      lock.withLock {
    ...
    public func resizeTerminal(_ newSize: ClairV2TerminalSize) throws {
      try lock.withLock {
        guard running, let handle else { throw ClairV2TerminalError.closed }
    ...
    private func finish(_ owned: clair_pty_handle, report: clair_pty_exit?, outputFailed: Bool) {
      ...
      let exit: ClairV2LocalShellExit = lock.withLock {
        let value = ClairV2LocalShellExit(
