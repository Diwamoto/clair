...
type MainView = 'workspace' | 'search' | 'source-control' | 'activity' | 'debug' | 'settings' | 'merge-graph' | 'session-rail';
...
type GitReviewScope = 'all' | 'committed' | 'uncommitted';
...
type ReviewFile = BranchFile & { commitHash?: string };
...
type AddAgentRequest = { agentId: string; launchTarget: AgentLaunchTarget; worktree: string; profile: 'review' | 'implement' | 'debug'; confirmChanges: boolean; initialPrompt: string };
...
type MergeGraphCommit = { hash: string; message: string; branch: string; branchColor: string; author: string; time: string };
type SessionRailRow = { id: string; agentId: string; agentLabel: string; statusLabel: string; statusTone: 'attention' | 'running' | 'idle' | 'exited'; projectId: string; context: string; elapsed: string; signal: string; usagePercent: number | null; action: string };
...
// ブランチレビュー（pane-split → main）。COMMIT済みはこのブランチで確定済みのコミットが
// 触れたファイル、未COMMITはワークツリーにある未追跡・未コミットのファイルを表す。
const sourceControlReview: { branch: string; base: string; commitCount: number; committedFiles: ReviewFile[]; uncommittedFiles: ReviewFile[] } = {
  branch: 'pane-split',
  base: 'main',
  commitCount: 3,
  committedFiles: [
    {
      path: 'apple/ClairApp/ProjectWorkspace.swift', kind: 'modified', additions: 42, deletions: 9, commitHash: 'a4f10c2',
      diff: [
        { old: 241, next: 241, kind: 'context', text: '  func updateSplitRatio(nodeID: UUID, ratio: Double) {' },
        { old: null, next: 242, kind: 'add', text: '    let clamped = min(max(ratio, 0.08), 0.92)' },
        { old: null, next: 243, kind: 'add', text: '    guard layout.ratio(of: nodeID) != clamped else {' },
        { old: null, next: 244, kind: 'add', text: '      return' },
        { old: null, next: 245, kind: 'add', text: '    }' },
        { old: 242, next: null, kind: 'remove', text: '    layout = layout.equalized()' },
        { old: null, next: 246, kind: 'add', text: '    layout = layout.settingRatio(clamped, for: nodeID)' },
        { old: null, next: 247, kind: 'add', text: '    persistWorkspace()' },
        { old: 243, next: 248, kind: 'context', text: '  }' },
        { old: 244, next: 249, kind: 'context', text: '' },
        { old: 245, next: 250, kind: 'context', text: '  /// Focus moves by geometry, not by pane index.' },
        { old: null, next: 251, kind: 'add', text: '  func focusPane(direction: PaneDirection) {' },
        { old: null, next: 252, kind: 'add', text: '    guard let next = layout.neighbor(' },
        { old: null, next: 253, kind: 'add', text: '      of: focusedPaneID, direction: direction' },
        { old: null, next: 254, kind: 'add', text: '    ) else { return }' },
        { old: null, next: 255, kind: 'add', text: '    focusPane(id: next)' },
        { old: null, next: 256, kind: 'add', text: '  }' },
        { old: 246, next: 257, kind: 'context', text: '}' },
      ],
    },
    { path: 'apple/ClairApp/PaneSplit.swift', kind: 'added', additions: 88, deletions: 0, commitHash: '7e2c9a1', diff: [
      { old: null, next: 1, kind: 'add', text: 'struct PaneSplit: View {' },
      { old: null, next: 2, kind: 'add', text: '    let node: ProjectPaneNode' },
      { old: null, next: 3, kind: 'add', text: '    let onFocus: (ProjectPaneNode.ID) -> Void' },
      { old: null, next: 4, kind: 'add', text: '}' },
    ] },
    { path: 'apple/ClairApp/ContentView.swift', kind: 'modified', additions: 16, deletions: 31, commitHash: '3b6f0d4', diff: [
      { old: 12, next: 12, kind: 'context', text: 'struct ContentView: View {' },
      { old: 13, next: null, kind: 'remove', text: '    let workspace: Workspace' },
      { old: null, next: 13, kind: 'add', text: '    @Environment(\\.workspace) private var workspace' },
      { old: 14, next: 14, kind: 'context', text: '' },
      { old: 15, next: 15, kind: 'context', text: '    var body: some View {' },
    ] },
    { path: 'apple/ClairApp/WorkspaceChrome.swift', kind: 'modified', additions: 7, deletions: 0, commitHash: '3b6f0d4', diff: [
      { old: 4, next: 4, kind: 'context', text: 'struct WorkspaceChrome: View {' },
      { old: null, next: 5, kind: 'add', text: '    var focusedPaneID: ProjectPaneNode.ID?' },
      { old: 5, next: 6, kind: 'context', text: '    let node: ProjectPaneNode' },
    ] },
    { path: 'apple/ClairTests/ProjectKernelTests.swift', kind: 'added', additions: 54, deletions: 0, commitHash: '5a2d9b6', diff: [
      { old: null, next: 1, kind: 'add', text: 'import Testing' },
      { old: null, next: 2, kind: 'add', text: '@testable import Clair' },
      { old: null, next: 3, kind: 'add', text: '' },
      { old: null, next: 4, kind: 'add', text: '@Test func focusPaneMovesByGeometry() { … }' },
    ] },
  ],
  uncommittedFiles: [
    { path: 'apple/ClairApp/SessionRail.swift', kind: 'added', additions: 0, deletions: 0, diff: [] },
  ],
};

// コミット履歴のブランチ/マージをグラフ表示する画面（SourceControlの「変更を確認」とは別画面）。
const mergeGraphCommits: MergeGraphCommit[] = [
  { hash: 'a4f10c2', message: 'Merge pane-split into main', branch: 'main', branchColor: '#5b88f7', author: 'diwamoto', time: '3時間前' },
  { hash: '7e2c9a1', message: 'Add focusPane keyboard nav', branch: 'pane-split', branchColor: '#8acb94', author: 'Claude', time: '5時間前' },
  { hash: '3b6f0d4', message: 'Split branch: pane-split', branch: 'main', branchColor: '#5b88f7', author: 'diwamoto', time: '1日前' },
  { hash: 'c88e1f3', message: 'Update installation guide', branch: 'docs-update', branchColor: '#8b6ac8', author: 'Claude', time: '2日前' },
  { hash: '5a2d9b6', message: 'Split branch: docs-update', branch: 'main', branchColor: '#5b88f7', author: 'diwamoto', time: '3日前' },
  { hash: '000f1a2', message: 'Initial commit', branch: 'main', branchColor: '#5b88f7', author: 'diwamoto', time: '2週間前' },
];

// 全Projectのセッション（terminal/agent）を1つの表にまとめた画面。ここに出す値はPTYと
// processから直接得られる事実のみ：stateはTerminalSession.State、signalはbell / exit code /
// 公式hook、実行contextは起動時に選ばれたcwd。TUIの内容は解釈しない。
const sessionRailRows: SessionRailRow[] = [
  { id: 'codex-clair', agentId: 'codex', agentLabel: '入力待ち', statusTone: 'attention', statusLabel: 'codex', projectId: 'clair', context: 'Project root', elapsed: '4m 12s', signal: 'terminal bell · 12:04:38', usagePercent: null, action: '移動 ↵' },
  { id: 'claude-clair', agentId: 'claude-code', agentLabel: '実行中', statusTone: 'running', statusLabel: 'Claude Code', projectId: 'clair', context: 'worktree · pane-split', elapsed: '18m 03s', signal: '公式hook: PostToolUse · 12:08:51', usagePercent: 38, action: '移動' },
  { id: 'claude-ccedit', agentId: 'claude-code', agentLabel: '実行中', statusTone: 'running', statusLabel: 'Claude Code', projectId: 'ccedit', context: 'worktree · retire-rust-core', elapsed: '1h 42m', signal: '公式hook: Notification · 11:31:02', usagePercent: 16, action: '移動' },
  { id: 'zsh-clair', agentId: 'zsh', agentLabel: '待機', statusTone: 'idle', statusLabel: 'zsh', projectId: 'clair', context: 'Project root', elapsed: '3h 06m', signal: '—', usagePercent: null, action: '移動' },
  { id: 'opencode-releases', agentId: 'opencode', agentLabel: 'exit 1', statusTone: 'exited', statusLabel: 'OpenCode', projectId: 'clair-releases', context: 'Project root', elapsed: '—', signal: 'exit code 1 · 11:58:14', usagePercent: null, action: '再起動' },
];
  ...
  return <div className="add-agent-overlay" role="presentation" onClick={onClose}>
    <section className="add-agent-window" role="dialog" aria-modal="true" aria-labelledby="add-agent-title" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="add-agent-body">
        ...
        <section className="add-agent-section"><header><strong>最初のプロンプト</strong><small>任意 · 空のまま起動すると待機状態で開きます。</small></header><textarea className="add-agent-prompt" value={initialPrompt} onChange={(event) => setInitialPrompt(event.target.value)} placeholder="例: EditorPane の行間を少し広げて、既存の配色は変えずに調整して。" rows={3} aria-label="最初のプロンプト" /></section>
      ...
      <footer className="add-agent-footer"><span className="add-agent-launch-summary"><VendorIcon agentId={selectedAgent.id} />{selectedAgent.name} · デフォルトモデル · {launchTarget === 'terminal' ? 'ターミナル' : 'アクティビティ'} · {worktreeMode === 'current' ? '現在のProject' : '新しいworktree'}</span><div><button type="button" className="add-agent-cancel" onClick={onClose}>キャンセル</button><button type="button" className="add-agent-primary" onClick={() => onStart({ agentId: selectedAgent.id, launchTarget, worktree, profile, confirmChanges, initialPrompt: initialPrompt.trim() })}>Agentを起動 <span>↗</span></button></div></footer>
    </section>
  </div>;
}
...
export default function Home() {
  ...
  const [selectedGitPath, setSelectedGitPath] = useState(sourceControlReview.committedFiles[0]?.path ?? '');
  const [gitReviewScope, setGitReviewScope] = useState<GitReviewScope>('all');
  ...
  const reviewFilesInScope = gitReviewScope === 'committed' ? sourceControlReview.committedFiles : gitReviewScope === 'uncommitted' ? sourceControlReview.uncommittedFiles : [...sourceControlReview.committedFiles, ...sourceControlReview.uncommittedFiles];
  const selectedReviewFile = reviewFilesInScope.find((file) => file.path === selectedGitPath) ?? reviewFilesInScope[0] ?? null;
  const selectedReviewIsCommitted = !!selectedReviewFile && sourceControlReview.committedFiles.some((file) => file.path === selectedReviewFile.path);
  const reviewScopeStats = reviewFilesInScope.reduce((totals, file) => ({ additions: totals.additions + file.additions, deletions: totals.deletions + file.deletions }), { additions: 0, deletions: 0 });
  ...
  const gitSidebar = <aside className="source-control-sidebar" aria-label="ソース管理"><header className="sidebar-header"><h2>変更を確認</h2><span className="change-count">{sourceControlReview.committedFiles.length + sourceControlReview.uncommittedFiles.length}</span></header><div className="change-list"><section className="change-section"><header><span>COMMIT済み</span><b>{sourceControlReview.commitCount} commits · {sourceControlReview.committedFiles.length} files</b></header>{sourceControlReview.committedFiles.map((file) => <button type="button" className={'change-row ' + (selectedReviewFile?.path === file.path ? 'is-active' : '')} key={file.path} onClick={() => setSelectedGitPath(file.path)}><span className={'change-kind ' + file.kind}>{file.kind[0].toUpperCase()}</span><span>{file.path}</span><small className="change-row-stat"><em className="is-add">+{file.additions}</em>{file.deletions > 0 && <em className="is-remove">−{file.deletions}</em>}</small></button>)}</section><section className="change-section"><header><span>未COMMIT</span><b>{sourceControlReview.uncommittedFiles.length} file{sourceControlReview.uncommittedFiles.length === 1 ? '' : 's'}</b></header>{sourceControlReview.uncommittedFiles.map((file) => <button type="button" className={'change-row ' + (selectedReviewFile?.path === file.path ? 'is-active' : '')} key={file.path} onClick={() => setSelectedGitPath(file.path)}><span className={'change-kind ' + file.kind}>{file.kind[0].toUpperCase()}</span><span>{file.path}</span><small className="change-row-stat is-untracked">未追跡</small></button>)}{sourceControlReview.uncommittedFiles.length === 0 && <p className="change-empty">ワークツリーはクリーンです</p>}</section></div></aside>;

  const activitySidebar = <aside className="activity-sidebar" aria-label="アクティビティ"><header className="sidebar-header"><h2>アクティビティ</h2><span className="activity-live-count">{activityThreads.length}</span></header><label className="activity-search"><span>⌕</span><input value={activityQuery} onChange={(event) => setActivityQuery(event.target.value)} placeholder="アクティビティを絞り込む" aria-label="アクティビティを絞り込む" /></label><div className="activity-project-filter" aria-label="Agentで絞り込み"><button type="button" className={activityAgentFilter === 'all' ? 'is-active' : ''} onClick={() => setActivityAgentFilter('all')}>すべて</button>{agentUsage.map((agent) => <button type="button" className={activityAgentFilter === agent.id ? 'is-active' : ''} key={agent.id} onClick={() => setActivityAgentFilter(agent.id)} aria-label={agent.name + 'で絞り込み'} title={agent.name}><VendorIcon agentId={agent.id} /></button>)}</div><div className="activity-list">{visibleThreads.map((thread) => <button type="button" key={thread.id} className={'activity-row ' + (thread.id === activitySelection ? 'is-active' : '')} onClick={() => { setActivitySelection(thread.id); setActivityReviewOpen(false); }}><span className={'activity-icon ' + thread.status}><VendorIcon agentId={thread.agentId} /></span><span><strong>{thread.summary}</strong><small>{thread.title} · {thread.projectId}</small></span><time>{thread.approval?.decision === 'pending' ? '承認待ち · ' : thread.approval ? '応答済み · ' : thread.review ? '変更 · ' : ''}{thread.updatedAt}</time></button>)}{visibleThreads.length === 0 && <p className="activity-empty">該当するアクティビティはありません</p>}</div></aside>;
  ...
  const renderGitSurface = () => <div className="diff-viewer"><header className="content-header diff-header"><div><span className="content-leading-icon">⑂</span><div><h1>ソース管理</h1><p>{sourceControlReview.branch} → {sourceControlReview.base} · {reviewFilesInScope.length}ファイル · +{reviewScopeStats.additions} −{reviewScopeStats.deletions}</p></div></div><div className="content-actions"><div className="git-view-switcher" role="tablist" aria-label="表示範囲"><button type="button" className={gitReviewScope === 'all' ? 'is-active' : ''} onClick={() => setGitReviewScope('all')}>全差分</button><button type="button" className={gitReviewScope === 'committed' ? 'is-active' : ''} onClick={() => setGitReviewScope('committed')}>commit済み</button><button type="button" className={gitReviewScope === 'uncommitted' ? 'is-active' : ''} onClick={() => setGitReviewScope('uncommitted')}>未commit</button></div><button type="button" disabled title="コンフリクトが検出された場合に有効になります">merge commitで採用</button><button type="button" onClick={() => openView('merge-graph')}>マージグラフを見る</button></div></header>{selectedReviewFile ? <><div className="diff-file-toolbar"><div><strong>{selectedReviewFile.path}</strong><span>{selectedReviewIsCommitted ? `commit済み · ${selectedReviewFile.commitHash ?? ''}` : '未COMMIT'} · {sourceControlReview.base} → {sourceControlReview.branch}</span></div></div>{selectedReviewFile.diff.length > 0 ? <><div className="diff-context"><span>@@ -7,3 +7,5 @@</span><b>{selectedReviewIsCommitted ? 'commit済み' : '未COMMIT'}</b></div><InlineDiff diff={selectedReviewFile.diff} /></> : <div className="empty-view"><span>＋</span><h1>新規ファイル</h1><p>{selectedReviewFile.path} はまだ追跡されていません。</p></div>}</> : <div className="empty-view"><span>✓</span><h1>ワークツリーはクリーンです</h1><p>{currentProject.name} に変更はありません。</p></div>}</div>;
  ...
  const mergeGraphLanes = [
    <svg width="34" height="32" viewBox="0 0 34 32" key="lane-0"><path d="M7 0V16" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><path d="M7 16V32" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><circle cx="7" cy="16" r="3.4" fill="#181b1f" stroke="#5b88f7" strokeWidth="1.8" /><path d="M17 0C17 8 7 8 7 16" stroke="#8acb94" strokeWidth="1.6" fill="none" /><path d="M27 0V32" stroke="#8b6ac8" strokeWidth="1.6" fill="none" /></svg>,
    <svg width="34" height="32" viewBox="0 0 34 32" key="lane-1"><path d="M7 0V32" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><path d="M17 0V32" stroke="#8acb94" strokeWidth="1.6" fill="none" /><circle cx="17" cy="16" r="3" fill="#8acb94" /><path d="M27 0V32" stroke="#8b6ac8" strokeWidth="1.6" fill="none" /></svg>,
    <svg width="34" height="32" viewBox="0 0 34 32" key="lane-2"><path d="M7 0V32" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><circle cx="7" cy="16" r="3" fill="#5b88f7" /><path d="M7 16C7 8 17 8 17 0" stroke="#8acb94" strokeWidth="1.6" fill="none" /><path d="M27 0V32" stroke="#8b6ac8" strokeWidth="1.6" fill="none" /></svg>,
    <svg width="34" height="32" viewBox="0 0 34 32" key="lane-3"><path d="M7 0V32" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><path d="M27 0V32" stroke="#8b6ac8" strokeWidth="1.6" fill="none" /><circle cx="27" cy="16" r="3" fill="#8b6ac8" /></svg>,
    <svg width="34" height="32" viewBox="0 0 34 32" key="lane-4"><path d="M7 0V32" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><path d="M27 0V32" stroke="#8b6ac8" strokeWidth="1.6" fill="none" /><circle cx="7" cy="16" r="3" fill="#5b88f7" /><path d="M7 16C7 8 27 8 27 0" stroke="#8b6ac8" strokeWidth="1.6" fill="none" /></svg>,
    <svg width="34" height="32" viewBox="0 0 34 32" key="lane-5"><path d="M7 0V32" stroke="#5b88f7" strokeWidth="1.6" fill="none" /><circle cx="7" cy="16" r="3.4" fill="#5b88f7" /></svg>,
  ];

  const renderMergeGraphSurface = () => <div className="diff-viewer merge-graph-view">
    <header className="content-header diff-header">
      <div><span className="content-leading-icon">⑂</span><div><h1>マージグラフ</h1><p>コミット履歴のブランチ / マージ · {sourceControlReview.branch} / {sourceControlReview.base}</p></div></div>
      <div className="content-actions merge-graph-legend">
        <span className="merge-graph-legend-item"><i style={{ background: '#5b88f7' }} />main</span>
        <span className="merge-graph-legend-item"><i style={{ background: '#8acb94' }} />pane-split</span>
        <span className="merge-graph-legend-item"><i style={{ background: '#8b6ac8' }} />docs-update</span>
        <button type="button" onClick={() => openView('source-control')}>変更を確認へ戻る</button>
      </div>
    </header>
    <div className="merge-graph-table" role="table" aria-label="コミット履歴">
      <div className="merge-graph-row merge-graph-heading" role="row"><span /><span>コミット</span><span>ブランチ</span><span>作者</span><span>日時</span><span>ハッシュ</span></div>
      {mergeGraphCommits.map((commit, index) => <div className="merge-graph-row" role="row" key={commit.hash}>
        <span className="merge-graph-lane">{mergeGraphLanes[index] ?? mergeGraphLanes[mergeGraphLanes.length - 1]}</span>
        <span className={'merge-graph-message' + (index === 0 ? ' is-merge' : '')}>{commit.message}</span>
        <span className="merge-graph-branch"><i style={{ background: commit.branchColor }} />{commit.branch}</span>
        <span className="merge-graph-author">{commit.author}</span>
        <span className="merge-graph-time">{commit.time}</span>
        <span className="merge-graph-hash">{commit.hash}</span>
      </div>)}
    </div>
  </div>;

  const renderSessionRailSurface = () => <div className="diff-viewer session-rail-view">
    <header className="content-header diff-header">
      <div><span className="content-leading-icon">◫</span><div><h1>セッション</h1><p>全Project · PTYとprocessから得られる事実のみ</p></div></div>
      <div className="content-actions">
        <span className="session-rail-hint">次の注意へ <kbd>⌥⇥</kbd></span>
        <button type="button" onClick={() => setAgentWindowOpen(true)}>Agentを起動 <kbd>⌃⌘N</kbd></button>
      </div>
    </header>
    <div className="session-rail-table" role="table" aria-label="全Projectのセッション">
      <div className="session-rail-row session-rail-heading" role="row"><span /><span>AGENT</span><span>PROJECT</span><span>実行CONTEXT</span><span>経過</span><span>最後のSIGNAL</span><span>利用枠</span><span /></div>
      {sessionRailRows.map((row) => <div className={'session-rail-row is-' + row.statusTone} role="row" key={row.id}>
        <span className="session-rail-status-dot" />
        <span className="session-rail-agent">{row.agentId === 'zsh' ? <SurfaceIcon kind="terminal" /> : <VendorIcon agentId={row.agentId} />}<strong>{row.statusLabel}</strong><em className="session-rail-chip">{row.agentLabel}</em></span>
        <span className="session-rail-project"><i />{row.projectId}</span>
        <span className="session-rail-context"><em className="session-rail-chip">{row.context}</em></span>
        <span className="session-rail-elapsed">{row.elapsed}</span>
        <span className="session-rail-signal">{row.signal}</span>
        <span className="session-rail-usage">{row.usagePercent !== null ? <><i><b style={{ width: row.usagePercent + '%' }} /></i><em>残り{row.usagePercent}%</em></> : <em className="is-muted">—</em>}</span>
        <span className="session-rail-action"><em className="session-rail-chip">{row.action}</em></span>
      </div>)}
    </div>
    <div className="session-rail-footnote"><span>ⓘ</span><p>ここに出す値は全て<b>PTYとprocessから直接得られる事実</b>である。stateは<code>TerminalSession.State</code>、signalはbell / exit code / 公式hook、実行contextは起動時に選ばれたcwd。TUIの内容は解釈しない。</p></div>
  </div>;

  return <main className="editor-app"><section className={'mac-window ' + (isResizing ? 'is-resizing' : '')}><header className="titlebar"><div className="traffic-lights" aria-hidden="true"><span /><span /><span /></div><nav className="project-tabstrip" aria-label="開いているエディタとターミナル">{projectGroups.map((group) => { const isActive = group.id === activeGroupId; return <div className={'project-group ' + (isActive ? 'is-active' : '')} style={{ '--group-color': group.color } as CSSProperties} key={group.id}><button type="button" className="project-group-label" onClick={() => activateProject(group.id)} title={group.name + ' · ' + group.root} aria-current={isActive ? 'page' : undefined}><span className="group-label-name">{group.name}</span></button><div className="project-surfaces" aria-label={group.name + ' のタブ'}><button type="button" className={'surface-tab surface-tab-editor ' + (isActive && activeSurface === 'editor' && mainView === 'workspace' ? 'is-active' : '')} onClick={() => selectSurface('editor', undefined, group.id)} title={group.activeFile}><SurfaceIcon kind="editor" /><span className="surface-tab-label">{group.activeFile}</span>{isActive && isDirty && <i className="tab-dirty-dot" />}</button>{group.terminals.map((terminal) => <button type="button" className={'surface-tab surface-tab-terminal ' + (isActive && activeSurface === 'terminal' && group.activeTerminalId === terminal.id && mainView === 'workspace' ? 'is-active' : '')} onClick={() => selectSurface('terminal', terminal.id, group.id)} title={terminal.label} key={terminal.id}><SurfaceIcon kind="terminal" /><span className="surface-tab-label">{terminal.label}</span></button>)}</div></div>; })}</nav><div className="title-actions"><button type="button" className="title-search" onClick={() => { setCommandOpen(true); setCommandQuery(''); setPaletteMode('quickOpen'); setPaletteIndex(0); }} aria-label="ファイル、シンボルを検索" title="ファイル、シンボルを検索"><span aria-hidden="true">⌕</span>ファイル、シンボル</button><button type="button" className={commandOpen ? 'is-active' : ''} onClick={() => { setCommandOpen((open) => !open); setCommandQuery(''); setPaletteMode('command'); setPaletteIndex(0); }} aria-label="コマンドウィンドウを開く" title="コマンドウィンドウ"><span aria-hidden="true">⌘</span></button><button type="button" className={mainView === 'settings' ? 'is-active' : ''} onClick={openSettings} aria-label="設定を開く" title="設定"><span aria-hidden="true">⚙︎</span></button></div></header>
