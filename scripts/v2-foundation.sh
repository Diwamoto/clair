...
core_package="$repo_root/packages/ClairV2Core"
...
check_v2_sources() {
  if rg -n --glob '*.swift' --glob 'Package.swift' \
    ...
    printf 'v2-foundation: v1 runtime dependency found in v2 packages.\n' >&2
    return 1
  fi
}
...
check() {
  ...
  check_v2_sources
  printf 'v2-foundation: package manifests and v1 dependency boundary passed.\n'
}
